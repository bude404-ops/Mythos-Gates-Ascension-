...
- Active Deity policy: ONE_PLAYER_CONTROLLED_DEITY
...
1. A clear player Deity start zone.
...
9. Enough flat/collision-safe tactical surface for one Deity, several smaller enemies, VFX, and camera motion.
10. No purely decorative layout that fails the mission objective.

## Artwork alignment
Use the mission art package as binding direction:

- Map prompt: Elongated 3D tactical map for First Light Under Dead Stone, sacred temple approach with shallow river-light lanes and low sandstone cover, readable mobile lanes, three-Deity tactical scale, faction-specific Aten Ra gold, sandstone, turquoise river-light, and solemn Gate pressure.
...
- Thumbnail promise: First Light Under Dead Stone thumbnail: three Deity silhouettes advancing through sacred temple approach with shallow river-light lanes and low sandstone cover.
...
Scale rules:
- Do not shrink Deities to fit the map; enlarge architecture and Gate landmarks.
- Normal enemies must read visibly smaller than the Deity.
...
- Deity attacks need room for arcs, shockwaves, dust, terrain reaction, and camera weight.
...
4. Scale comparison preview with player Deity, standard enemy, Gate arch, cover, and lane width.
...
- Bottom action bar obstruction must not hide the objective, player Deity, or enemy threats.
...
- The Gate is too small to justify Deity travel.
- Deities look human-sized because the terrain or camera is wrong.
