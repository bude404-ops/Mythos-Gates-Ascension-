...
export function summarizeCrossFactionRunIns({ factions, creatures, pools, missions, campaigns }) {
  ...
  return {
    ...
    oneDeityMissions: missions.filter(m => m.activeDeityPolicy?.standardCombat === ONE_DEITY_RULE).length,
    mobileCampaigns: campaigns.filter(c => c.mobileUe5Architecture?.combatRule === ONE_DEITY_RULE).length
  };
}
