# MYTHOS GATES: ASCENSION — COMPLETE FACTION STAT SHEETS
**Version:** 1.0  
**Date:** August 19, 2026  
**Total Entries:** 28 Deities + 39 Creatures + 4 World Bosses = 71 Assets  

> **Avatar System**: Players channel Avatars — divine projections of their chosen Deity. The Deity remains protected in its domain. Progression via Belief and Influence. Avatar death = respawn. Solo combat. God-scale in all realms.


---

## STAT LEGEND

| Stat | Abbrev | Description |
|------|--------|-------------|
| HP | Health Points | Damage capacity before death |
| DEF | Defense | Damage reduction (%) |
| ATK | Attack | Base damage scaling |
| DE | Divine Energy | Ability resource pool |
| SPD | Speed | Movement + attack speed (1-10) |
| CRIT | Critical Rate | Critical hit chance (%) |
| PEN | Penetration | Ignores enemy DEF |
| REC | Recovery | HP regen per second |
| AREA | Area | AoE radius modifier (1-10) |
| DMG TYPE | Damage Type | Primary damage element |
| ROLE | Combat Role | Playstyle archetype |

---

# SECTION 1: PLAYABLE DEITIES (28)

## FACTION 1: ATEN RA — The Solar Dominion of Khepra
**Faction Damage Type:** Solar  
**Faction Skin:** Electrum-Gold  
**Faction Aura:** Amber-Gold  

### 001. Aten-Ra — Endurance
**Mythology:** Ra/Aten/Ma'at — the noon sun authority, living solar judgment  
| Stat | Value |
|------|-------|
| HP | 17500 |
| DEF | 400 (45% reduction) |
| ATK | 380 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Solar |
| ROLE | Warrior |

**Passive — Solar Authority:** Enemies within 5m take -15% damage. Aten-Ra cannot be moved by knockback.  
**Ability 1 — Sun-Disc Strike:** Forward arc solar slash. 220 ATK damage. 3s cooldown.  
**Ability 2 — Ma'at Shield:** Absorbs next 3 hits. Reflected as 150 ATK Solar damage. 8s cooldown.  
**Ability 3 — Noon Verdict:** Radiates Solar burst. 400 ATK AoE. Marks all hit enemies for +25% damage. 14s cooldown.  
**Ultimate — Living Sun:** Transforms into solar form for 8s. All attacks become AoE Solar. +50% ATK. Immune to status effects.  

### 002. Sutekh — Conduit
**Mythology:** Khepri — scarab dawn engine, renewal and rebirth  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Solar |
| ROLE | Caster |

**Passive — Dawn Engine:** Every 3rd ability cast refunds 30% DE cost and reduces all cooldowns by 1s.  
**Ability 1 — Scarab Roll:** Dash forward rolling enemies. 180 ATK + knockback. Generates 40 DE. 2s cooldown.  
**Ability 2 — Renewal Cycle:** Heal 8% max HP. Next ability costs 50% less DE. 6s cooldown.  
**Ability 3 — Dawn Burst:** Detonate stored solar energy. 350 ATK AoE. Scales with current DE. 12s cooldown.  
**Ultimate — Rebirth:** Full HP heal + DE refill. Next 3 abilities have 0 cooldown for 6s.  

### 003. Iset — Dominion
**Mythology:** Nephthys/Duat — necropolis dawn, horizon threshold judge  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Spirit |
| ROLE | Archer |

**Passive — Threshold Judge:** Enemies below 30% HP are Slowed and cannot use abilities.  
**Ability 1 — Duat Gate:** Place a necropolis zone. Enemies inside take 80 ATK/s Spirit damage. Lasts 6s. 5s cooldown.  
**Ability 2 — Horizon Pull:** Pull all enemies toward a target point. 120 ATK. 7s cooldown.  
**Ability 3 — Necropolis Verdict:** All enemies in zone take 300 ATK Spirit damage. If killed, zone expands. 11s cooldown.  
**Ultimate — Between Worlds:** Freezes time for 3s. Only Iset can act. All enemies Stunned.  

### 004. Amunet — Fracture
**Mythology:** Ma'at weighing/punishment — prism of divine verdict  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Solar |
| ROLE | Assassin |

**Passive — Divine Verdict:** Every 4th hit ignores all DEF and applies Stagger.  
**Ability 1 — Prism Slash:** Forward thrust. 250 ATK. Reduces enemy DEF by 20% for 4s. 3s cooldown.  
**Ability 2 — Weighing Scale:** Lock enemy in place. Deals 180 ATK. If enemy has more HP, deals +50% damage. 8s cooldown.  
**Ability 3 — Verdict Shatter:** Heavy overhead. 450 ATK. Destroys enemy shields/barriers. 13s cooldown.  
**Ultimate — Ma'at Final Judgment:** Channels for 2s. Deals 800 ATK to all enemies. Enemies above 50% HP take double.  

---

## FACTION 2: ASGARDIAN — The Storm-Rooted Aesir Holds
**Faction Damage Type:** Storm  
**Faction Skin:** Warm Gunmetal  
**Faction Aura:** Blue-White Lightning  

### 005. Odin — Endurance
**Mythology:** Odin — Odin, wisdom, war, sacrifice  
| Stat | Value |
|------|-------|
| HP | 18000 |
| DEF | 420 (47% reduction) |
| ATK | 360 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Storm |
| ROLE | Caster |

**Passive — Raven Wisdom:** Odin sees enemy telegraphs 0.3s earlier. 10% chance to dodge any attack.  
**Ability 1 — Storm Spear:** Throw a lightning spear. 200 ATK Storm. Chains to 2 nearby enemies. 3s cooldown.  
**Ability 2 — Oath Shield:** Raise shield. Block all frontal damage for 3s. Reflected as 150 ATK Storm. 8s cooldown.  
**Ability 3 — Sacrifice Eye:** Lose 15% max HP. Gain +80% ATK for 5s. Cannot die during this window. 14s cooldown.  
**Ultimate — Throne of Storms:** Summon a storm throne. All enemies within 8m take 60 ATK/s Storm damage. Odin is immune to CC. Lasts 8s.  

### 006. Thor — Conduit
**Mythology:** Thor — thunder, strength, protection  
| Stat | Value |
|------|-------|
| HP | 12000 |
| DEF | 200 |
| ATK | 460 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 20 |
| AREA | 6 |
| DMG TYPE | Storm |
| ROLE | Warrior |

**Passive — Thunder Charge:** Every hit builds a charge. At 5 charges, next ability deals +50% damage and refunds DE.  
**Ability 1 — Hammer Throw:** Throw Mjolnir. 220 ATK Storm. Returns to hand. 3s cooldown.  
**Ability 2 — Thunder Clap:** AoE Storm burst. 160 ATK + Stun 1.5s. Generates 50 DE. 5s cooldown.  
**Ability 3 — Lightning Lance:** Long-range Storm beam. 380 ATK. Pierces all enemies in line. 12s cooldown.  
**Ultimate — God of Thunder:** Thor channels lightning for 6s. All attacks become AoE Storm +30% ATK. Storm chain lightning on every hit.  

### 007. Skadi — Dominion
**Mythology:** Heimdall — guardian, vigilance, the bridge  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Storm |
| ROLE | Archer |

**Passive — Bridge Watch:** Enemies that enter Skadi's range are Marked (+25% damage taken) for 4s.  
**Ability 1 — Gjallar Horn:** Sound wave. 150 ATK. Pushes enemies back and Silences for 2s. 5s cooldown.  
**Ability 2 — Rainbow Bridge:** Create a storm-bridge path. Skadi moves 50% faster on it. Enemies on it take 60 ATK/s. 7s cooldown.  
**Ability 3 — Vigilant Strike:** 320 ATK to Marked target. If target is Marked, consumes mark for +100% damage. 10s cooldown.  
**Ultimate — Bifrost Shatter:** Shatters the ground in a massive line. 500 ATK Storm. All enemies in path are Stunned 3s.  

### 008. Freyja — Fracture
**Mythology:** Freyja — Vanir goddess, seiðr magic, war/love  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Storm |
| ROLE | Assassin |

**Passive — Seiðr Fury:** Every 3rd consecutive hit on the same enemy applies Armor Break (-25% DEF, stacks 2x).  
**Ability 1 — Seiðr Lash:** Forward whip-storm. 250 ATK. Applies Armor Break. 3s cooldown.  
**Ability 2 — War Frenzy:** Freyja gains +40% ATK and +30% SPD for 5s. 8s cooldown.  
**Ability 3 — Falcon Dive:** Dash through enemies. 400 ATK to all passed through. Leaves a storm trail (60 ATK/s). 12s cooldown.  
**Ultimate — Valhalla's Call:** Summons 3 Valkyrie spirits. Each deals 200 ATK Storm per hit. Lasts 8s. Freyja gains lifesteal.  

---

## FACTION 3: OLYMPIAN — The Celestial Heights of Olympus
**Faction Damage Type:** Divine  
**Faction Skin:** Living Marble  
**Faction Aura:** Gold-Blue Skyfire  

### 009. Zeus — Endurance
**Mythology:** Zeus — sky father, lightning, sovereignty  
| Stat | Value |
|------|-------|
| HP | 17500 |
| DEF | 400 (45% reduction) |
| ATK | 380 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Divine |
| ROLE | Warrior |

**Passive — Sky Sovereignty:** Zeus takes 20% less damage from enemies above 50% HP. Immune to knockback.  
**Ability 1 — Thunderbolt:** Call down a bolt. 220 ATK Divine. 3s cooldown.  
**Ability 2 — Aegis Shield:** Block all damage for 2s. Next attack deals +50% damage. 8s cooldown.  
**Ability 3 — Sky Throne:** Create a zone. Zeus is immune to CC inside. Enemies take 100 ATK/s Divine. 14s cooldown.  
**Ultimate — Olympus Decree:** 700 ATK Divine AoE. All enemies are Stunned 2.5s. Zeus is fully healed.  

### 010. Athena — Conduit
**Mythology:** Apollo — light, prophecy, archery, music  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Divine |
| ROLE | Caster |

**Passive — Lyre of Light:** Every ability generates a light note. 3 notes = next ability free + +30% damage.  
**Ability 1 — Sun Arrow:** Long-range Divine arrow. 200 ATK. Pierces. 2s cooldown.  
**Ability 2 — Prophetic Vision:** Mark all enemies in a large area. Marked enemies take +25% damage for 5s. 6s cooldown.  
**Ability 3 — Chorus Strike:** 3 rapid arrows. 150 ATK each. Each applies Burn (30 ATK/s for 3s). 10s cooldown.  
**Ultimate — Delphic Hymn:** AoE Divine wave. 500 ATK. All enemies are Marked + Burned. Athena refills DE.  

### 011. Artemis — Dominion
**Mythology:** Athena — wisdom, strategy, just war  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Divine |
| ROLE | Archer |

**Passive — Strategic Mind:** Artemis gains +10% damage for each enemy within 8m (max 40%).  
**Ability 1 — Phalanx Wall:** Create a Divine barrier. Blocks enemy movement and projectiles. 5s cooldown.  
**Ability 2 — Aegis Reflect:** Deflect next attack back at enemy for 200 ATK. 7s cooldown.  
**Ability 3 — Spear of Wisdom:** 350 ATK line-pierce. Stuns first enemy hit for 2s. 11s cooldown.  
**Ultimate — Strategos:** For 6s, all enemy telegraphs are visible. Artemis gains +50% ATK and all abilities cost 50% DE.  

### 012. Ares — Fracture
**Mythology:** Ares — war, violence, courage  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Divine |
| ROLE | Assassin |

**Passive — Bloodlust:** Every kill restores 15% HP and 30 DE.  
**Ability 1 — War Slash:** Heavy slash. 260 ATK. Applies Armor Break. 3s cooldown.  
**Ability 2 — Phobos Fear:** Enemies in cone are Feared (flee) for 2s. 7s cooldown.  
**Ability 3 — Spear of Mars:** 450 ATK charge. Ignores DEF. 13s cooldown.  
**Ultimate — God of War:** 8s frenzy. +60% ATK, +40% SPD, lifesteal 20%. Cannot be CC'd.  

---

## FACTION 4: KAMI — The Shrine-Path Archipelago
**Faction Damage Type:** Spirit  
**Faction Skin:** Living Lacquer  
**Faction Aura:** Foxfire Blue  

### 013. Amaterasu — Endurance
**Mythology:** Amaterasu — sun goddess, the shining heaven  
| Stat | Value |
|------|-------|
| HP | 17500 |
| DEF | 400 (45% reduction) |
| ATK | 380 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Spirit |
| ROLE | Warrior |

**Passive — Radiant Dawn:** Amaterasu heals 2% max HP/s in combat. Enemies within 4m take -10% ATK.  
**Ability 1 — Mirror Strike:** 220 ATK Spirit. Reflects 30% of damage taken next 2s back at attacker. 3s cooldown.  
**Ability 2 — Sacred Mirror:** Place a mirror. Deflects enemy projectiles. 8s cooldown.  
**Ability 3 — Sun Disc Throw:** 380 ATK AoE Spirit. Blinds enemies (50% miss chance) for 3s. 14s cooldown.  
**Ultimate — Heaven's Light:** Massive Spirit burst. 650 ATK. Heals Amaterasu for 25% max HP. Blinds all enemies 3s.  

### 014. Tsukuyomi — Conduit
**Mythology:** Tsukuyomi — moon god, night, order  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Spirit |
| ROLE | Caster |

**Passive — Moon Phase:** Alternates between Waxing (+30% ATK) and Waning (+30% DEF) every 8s.  
**Ability 1 — Moonbeam Slash:** 200 ATK Spirit. During Waxing, +50% damage. 2s cooldown.  
**Ability 2 — Lunar Veil:** Become invisible for 2s. Next attack deals +100% damage. 6s cooldown.  
**Ability 3 — Eclipse:** 350 ATK AoE. During Waning, also Silences enemies for 2s. 12s cooldown.  
**Ultimate — Tsukuyomi's Domain:** 6s. Night falls. All enemies lose 30% accuracy. Tsukuyomi gains +50% ATK and full DE refill.  

### 015. Susanoo — Dominion
**Mythology:** Raijin — thunder, storms, chaos  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Storm |
| ROLE | Archer |

**Passive — Drum of Storms:** Every 4th attack triggers a free storm bolt (150 ATK) at nearest enemy.  
**Ability 1 — Thunder Drum:** 180 ATK Storm AoE. Stuns for 1s. 4s cooldown.  
**Ability 2 — Storm Path:** Create a lightning trail. Enemies on it take 80 ATK/s and are Slowed. 7s cooldown.  
**Ability 3 — Raijin's Fury:** 340 ATK Storm to all enemies in a large cone. 11s cooldown.  
**Ultimate — Tempest God:** 8s. Storm fills the room. All enemies take 100 ATK/s Storm. Random chain lightning every 2s.  

### 016. Izanami — Fracture
**Mythology:** Susanoo — storm god, the impetuous  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Storm |
| ROLE | Assassin |

**Passive — Storm Blade:** Every hit applies a stack of Storm Charge. At 3 stacks, next attack deals +200% damage and consumes all stacks.  
**Ability 1 — Tsurugi Slash:** 260 ATK Storm. Applies 1 Storm Charge stack. 3s cooldown.  
**Ability 2 — Whirlwind Cut:** Spin attack. 180 ATK to all around. Knocks back. 6s cooldown.  
**Ability 3 — Dragon Slayer:** 450 ATK Storm. If target has 3 Storm Charges, deals +100% and Stuns. 13s cooldown.  
**Ultimate — Eight-Headed Storm:** 800 ATK Storm AoE. Clears all enemies in a massive area. Izanami is invincible during animation.  

---

## FACTION 5: TUATHA — The Root-Crowned Green Below
**Faction Damage Type:** Nature  
**Faction Skin:** Living Wood-Bark  
**Faction Aura:** Silver-Green Mist  

### 017. Dagda — Endurance
**Mythology:** The Dagda — good god, abundance, life/death  
| Stat | Value |
|------|-------|
| HP | 18000 |
| DEF | 420 (47% reduction) |
| ATK | 360 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 30 |
| AREA | 3 |
| DMG TYPE | Nature |
| ROLE | Warrior |

**Passive — The Dagda's Cauldron:** Dagda regenerates 30 HP/s. Cannot drop below 1 HP from a single hit (survives at 1 HP once per 30s).  
**Ability 1 — Club of Life:** 200 ATK Nature. Heals Dagda for 5% max HP. 3s cooldown.  
**Ability 2 — Harp of Seasons:** AoE that Slows enemies 40% and heals Dagda 8% HP over 4s. 8s cooldown.  
**Ability 3 — Earth Pillar:** 380 ATK Nature. Spawns a stone pillar that blocks movement and projectiles. 14s cooldown.  
**Ultimate — Cauldron of Rebirth:** Full heal. Next death within 10s triggers instant respawn at 50% HP.  

### 018. Brigid — Conduit
**Mythology:** Lugh — many-skilled, light, champion  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Nature |
| ROLE | Caster |

**Passive — Master of All Arts:** Every 5th ability is free. Abilities used in sequence deal +10% (stacking, max +50%).  
**Ability 1 — Spear of Lugh:** Throw a divine spear. 220 ATK. Returns. 2s cooldown.  
**Ability 2 — Swift Strike:** Dash forward. 160 ATK to enemies passed. +50% SPD for 2s. 5s cooldown.  
**Ability 3 — Solas Burst:** 350 ATK Nature AoE. Generates 80 DE if it hits 3+ enemies. 11s cooldown.  
**Ultimate — Lugh Samildánach:** 8s. All abilities cost 0 DE. Every ability cast grants +15% ATK (stacking).  

### 019. Morrigan — Dominion
**Mythology:** Brigid — flame, poetry, healing, forge  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Nature |
| ROLE | Archer |

**Passive — Sacred Flame:** Morrigan's attacks apply Burn (40 ATK/s for 3s, stacks 2x).  
**Ability 1 — Flame Lash:** 180 ATK Nature + Burn. Chains to 2 nearby enemies. 4s cooldown.  
**Ability 2 — Forge Wall:** Create a fire wall. Enemies passing through take 100 ATK/s Burn. Blocks movement. 7s cooldown.  
**Ability 3 — Poetic Fire:** 320 ATK AoE. All enemies with Burn take +50% damage from this ability. 11s cooldown.  
**Ultimate — Brigid's Hearth:** 10s. Creates a sacred zone. Morrigan and allies heal 5% HP/s. Enemies take 150 ATK/s.  

### 020. Lugh — Fracture
**Mythology:** Morrigan — phantom queen, war, fate, death  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Nature |
| ROLE | Assassin |

**Passive — Battle Crow:** Enemies below 25% HP are revealed. Lugh deals +50% damage to enemies below 25% HP.  
**Ability 1 — Crow Slash:** 260 ATK Nature. Applies Marked (+25% damage taken) for 4s. 3s cooldown.  
**Ability 2 — Phantom Shift:** Teleport behind target enemy. 120 ATK. 6s cooldown.  
**Ability 3 — Death Omen:** 450 ATK. If target dies within 3s, Lugh gains +50% ATK for 5s. 13s cooldown.  
**Ultimate — The Morrigan's Cry:** 700 ATK Nature AoE. All enemies Feared for 2s. Lugh gains lifesteal 25% for 6s.  

---

## FACTION 6: EMPYREAN — The White Citadel Above Law
**Faction Damage Type:** Divine  
**Faction Skin:** Living Light  
**Faction Aura:** White-Gold Radiance  

### 021. Michael — Endurance
**Mythology:** Michael — warrior archangel, divine protector  
| Stat | Value |
|------|-------|
| HP | 17500 |
| DEF | 400 (45% reduction) |
| ATK | 380 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Divine |
| ROLE | Warrior |

**Passive — Containment:** Michael's armor contains his radiance. When below 30% HP, releases a radiant burst (300 ATK AoE) and heals 10% HP. 30s cooldown.  
**Ability 1 — Radiant Blade:** 220 ATK Divine. Pierces. 3s cooldown.  
**Ability 2 — Shield of Light:** Absorb 3 hits. Reflect as 150 ATK Divine. 8s cooldown.  
**Ability 3 — Judgement Cut:** 380 ATK Divine. Stuns target for 1.5s. 14s cooldown.  
**Ultimate — Seraphic Wrath:** 650 ATK Divine AoE. All enemies Stunned 2s. Michael gains invulnerability for 3s.  

### 022. Gabriel — Conduit
**Mythology:** Gabriel — messenger, annunciation, divine voice  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Divine |
| ROLE | Caster |

**Passive — Divine Messenger:** Every ability cast grants 10% move speed for 3s (stacks 3x).  
**Ability 1 — Herald Strike:** 200 ATK Divine. Marks target for +25% damage. 2s cooldown.  
**Ability 2 — Trumpet of Heaven:** AoE Divine. 160 ATK. Silences all enemies for 2s. 5s cooldown.  
**Ability 3 — Annunciation:** 350 ATK Divine to Marked targets. Refunds 50 DE if it hits Marked enemy. 10s cooldown.  
**Ultimate — Gabriel's Proclamation:** 550 ATK Divine AoE. All enemies Marked + Silenced 3s. Gabriel refills DE.  

### 023. Raphael — Dominion
**Mythology:** Raphael — healer, guide, divine medicine  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Divine |
| ROLE | Archer |

**Passive — Healing Light:** Raphael heals 3% max HP every 5s. Abilities that hit enemies also heal Raphael for 5% max HP.  
**Ability 1 — Light Bind:** 180 ATK Divine. Roots enemy in place for 2s. 4s cooldown.  
**Ability 2 — Purifying Wave:** Cleanse all negative effects. Heal 10% HP. Push enemies back. 7s cooldown.  
**Ability 3 — Guiding Light:** 320 ATK Divine line. All enemies hit are Blinded for 3s. 11s cooldown.  
**Ultimate — Raphael's Sanctuary:** 8s zone. Raphael is immune to damage. Heals 5% HP/s. Enemies inside take 100 ATK/s Divine.  

### 024. Jophiel — Fracture
**Mythology:** Jophiel — fire of God, punishment, repentance  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Divine |
| ROLE | Assassin |

**Passive — Fire of God:** Every 3rd hit applies Burn (50 ATK/s for 4s). Burn stacks 2x.  
**Ability 1 — Flame Sword:** 260 ATK Divine + Burn. 3s cooldown.  
**Ability 2 — Repentance:** Enemies take 200 ATK Divine. If they have Burn, Stuns for 2s and consumes Burn. 7s cooldown.  
**Ability 3 — Divine Fire:** 450 ATK Divine AoE. Applies Burn to all hit. 13s cooldown.  
**Ultimate — Jophiel's Conflagration:** 800 ATK Divine in a massive cone. All enemies Burned for 5s. Jophiel gains +40% ATK for 6s.  

---

## FACTION 7: INFERNAL DOMINION — The Black Iron Court
**Faction Damage Type:** Infernal  
**Faction Skin:** Living Ash  
**Faction Aura:** Ember-Red Anti-Radiance  

### 025. Lucifer — Endurance
**Mythology:** Lucifer — morning star, fallen light, pride  
| Stat | Value |
|------|-------|
| HP | 18000 |
| DEF | 420 (47% reduction) |
| ATK | 360 |
| DE | 200 |
| SPD | 4 |
| CRIT | 5% |
| PEN | 50 |
| REC | 25 |
| AREA | 3 |
| DMG TYPE | Infernal |
| ROLE | Warrior |

**Passive — Morning Star:** Lucifer absorbs light around him. Enemies within 5m deal -15% damage (darkness debuff).  
**Ability 1 — Shadow Blade:** 220 ATK Infernal. Reduces enemy healing by 50% for 4s. 3s cooldown.  
**Ability 2 — Dark Aegis:** Block all damage for 2s. Counter with 200 ATK Infernal. 8s cooldown.  
**Ability 3 — Fallen Crown:** 380 ATK Infernal AoE. All enemies are Feared for 1.5s. 14s cooldown.  
**Ultimate — Light Devourer:** 700 ATK Infernal. Absorbs all enemy buffs. Lucifer heals for each buff absorbed. 8s aura of darkness.  

### 026. Asmodeus — Conduit
**Mythology:** Asmodeus — king of demons, desire, destruction  
| Stat | Value |
|------|-------|
| HP | 11000 |
| DEF | 180 |
| ATK | 420 |
| DE | 480 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 50 |
| REC | 20 |
| AREA | 5 |
| DMG TYPE | Infernal |
| ROLE | Caster |

**Passive — Hellfire Engine:** Every ability generates 1 Hellfire charge. 3 charges = next ability deals +75% damage and costs 0 DE.  
**Ability 1 — Spike Fist:** 220 ATK Infernal. Knockback. 2s cooldown.  
**Ability 2 — Infernal Chains:** Pull 3 enemies to Lucifer. 120 ATK. Generates 60 DE. 5s cooldown.  
**Ability 3 — Hellfire Nova:** 350 ATK Infernal AoE. Applies Burn. 12s cooldown.  
**Ultimate — King of Demons:** 8s. All abilities cost 0 DE. Every ability cast spawns a hellfire nova (200 ATK AoE).  

### 027. Lilith — Dominion
**Mythology:** Lilith — night, seduction, independence  
| Stat | Value |
|------|-------|
| HP | 11500 |
| DEF | 200 |
| ATK | 440 |
| DE | 300 |
| SPD | 6 |
| CRIT | 10% |
| PEN | 60 |
| REC | 15 |
| AREA | 7 |
| DMG TYPE | Infernal |
| ROLE | Archer |

**Passive — Night Touched:** Enemies that hit Lilith are Charming (walk toward her for 1.5s). 8s cooldown per enemy.  
**Ability 1 — Shadow Veil:** 180 ATK Infernal. Lilith becomes invisible for 2s. 4s cooldown.  
**Ability 2 — Dark Allure:** Create a shadow zone. Enemies inside are Slowed 50% and take 80 ATK/s Infernal. 7s cooldown.  
**Ability 3 — Seduction:** 320 ATK Infernal to nearest enemy. Charms for 3s (fights for Lilith). 11s cooldown.  
**Ultimate — Lilith's Embrace:** 8s. Night falls. All enemies are Charming. Lilith gains +50% ATK and lifesteal 25%.  

### 028. Naamah — Fracture
**Mythology:** Naamah — succubus, desire, corruption  
| Stat | Value |
|------|-------|
| HP | 12500 |
| DEF | 230 |
| ATK | 560 |
| DE | 200 |
| SPD | 6 |
| CRIT | 15% |
| PEN | 150 |
| REC | 10 |
| AREA | 4 |
| DMG TYPE | Infernal |
| ROLE | Assassin |

**Passive — Velvet Corruption:** Every hit applies Corrupted (healing reversed) for 2s. Stacks 2x.  
**Ability 1 — Shadow Claw:** 260 ATK Infernal. Applies Corrupted. 3s cooldown.  
**Ability 2 — Desire Drain:** 200 ATK Infernal. Heals Naamah for 100% of damage dealt. 6s cooldown.  
**Ability 3 — Corrupt Blade:** 450 ATK Infernal. If target is Corrupted, deals +100% and spreads Corrupted to nearby enemies. 13s cooldown.  
**Ultimate — Naamah's Hunger:** 800 ATK Infernal AoE. All enemies Corrupted for 5s. Naamah gains lifesteal 40% for 6s.  

---

# SECTION 2: CREATURE STAT SHEETS (39)

## HOLLOW FACTION CREATURES (001-018)
**Faction Damage Type:** Void  
**Faction Visual:** Void-mist bodies, white fracture cracks, anti-light, Gate stone fragments  

### Creature Scaling Reference
| Stat | Trash | Elite | Mini-Boss | Boss |
|------|-------|-------|-----------|------|
| HP | 2000-3500 | 5000-8000 | 7500-12000 | 12000-20000 |
| ATK | 80-150 | 180-280 | 280-400 | 400-600 |
| DEF | 20-50 | 80-120 | 120-180 | 180-280 |
| SPD | 3-7 | 4-7 | 4-6 | 3-5 |
| DE | 0 | 100-150 | 150-200 | 200-300 |

### 001. Hollow Wretch — Trash
| Stat | Value |
|------|-------|
| HP | 2200 |
| ATK | 100 |
| DEF | 30 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Void Swipe:** Basic melee. Applies Hollowed (50% DE cost increase) for 2s on hit.  
**Behavior:** Swarms in groups of 3-5. No telegraph. Dies fast.  

### 002. Gateborn Colossus — Elite
| Stat | Value |
|------|-------|
| HP | 6500 |
| ATK | 220 |
| DEF | 100 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Stone Fist:** Heavy telegraphed punch. 280 ATK. Knockback.  
**Ability — Gate Fragment:** Throws a piece of its own Gate stone. 180 ATK ranged.  
**Behavior:** Slow, hits hard. Telegraphs all attacks. Resistant to knockback.  

### 003. Beast Realm Maneater — Elite
| Stat | Value |
|------|-------|
| HP | 6000 |
| ATK | 250 |
| DEF | 80 |
| SPD | 6 |
| DMG TYPE | Void |
**Ability — Pounce:** Leaps at deity. 200 ATK. Knocks down.  
**Ability — Devour:** If deity below 20% HP, executes.  
**Behavior:** Aggressive, fast. Circles before pouncing. Weak to stagger.  

### 004. Forgotten Standard Bearer — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 200 |
| DEF | 130 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Banner of Forgetting:** Aura. Allies within 6m gain +50% ATK and are immune to Stun.  
**Ability — Memory Lash:** 180 ATK. Applies Hollowed for 4s.  
**Behavior:** Stays back. Buffs other Hollow. Priority target.  

### 005. Leviathan of the First Flood — World Boss (See World Boss Section)

### 006. Hollow Choirling — Trash
| Stat | Value |
|------|-------|
| HP | 1800 |
| ATK | 80 |
| DEF | 20 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Dissonant Wail:** Ranged. 60 ATK. Silences for 1s.  
**Behavior:** Stays at range. Wails in groups. Very fragile.  

### 007. Hollow Knell Archer — Trash
| Stat | Value |
|------|-------|
| HP | 2000 |
| ATK | 120 |
| DEF | 25 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Void Arrow:** Ranged. 100 ATK. Pierces. Applies Slow 20% for 2s.  
**Behavior:** Keeps distance. Fires in volleys of 3. Repositions when approached.  

### 008. Hollow Oath-Eater — Elite
| Stat | Value |
|------|-------|
| HP | 7500 |
| ATK | 240 |
| DEF | 110 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Oath Devour:** Consumes a buff from the deity. Heals self for 1500 HP.  
**Ability — Broken Jaw Slam:** 300 ATK. Stuns 1.5s.  
**Behavior:** Prioritizes buffed targets. Heavy hitter. Telegraphs jaw slam.  

### 009. Hollow Chain Auditor — Elite
| Stat | Value |
|------|-------|
| HP | 6800 |
| ATK | 210 |
| DEF | 120 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Chain Bind:** Roots deity for 2s. 120 ATK.  
**Ability — Audit:** Counts deity's DE. If above 50%, deals +50% damage for 5s.  
**Behavior:** Controls tempo. Binds then punishes resource-heavy play.  

### 010. Hollow Laurel Butcher — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 260 |
| DEF | 90 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Cleaver Swing:** 280 ATK. Wide arc. Applies Bleed (40 ATK/s for 3s).  
**Ability — Trophy Cut:** If deity is Bleeding, deals +50% damage.  
**Behavior:** Aggressive melee. Hits hard but low DEF. Telegraphs cleaver.  

### 011. Hollow Sun-Priest Shade — Elite
| Stat | Value |
|------|-------|
| HP | 6000 |
| ATK | 230 |
| DEF | 100 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Stolen Prayer:** 180 ATK AoE. Heals nearby Hollow for 500 HP each.  
**Ability — False Solar:** 200 ATK. Looks like Solar damage but is Void.  
**Behavior:** Support caster. Heals Hollow group. Priority target.  

### 012. Hollow Storm-Drinker — Elite
| Stat | Value |
|------|-------|
| HP | 7200 |
| ATK | 220 |
| DEF | 110 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Storm Absorb:** Immune to Storm damage. Converts Storm damage into HP heal.  
**Ability — Void Discharge:** 260 ATK AoE. Released after absorbing 3 Storm hits.  
**Behavior:** Counter-picks Storm deities. Bait then discharge.  

### 013. Hollow Mirror-Face — Elite
| Stat | Value |
|------|-------|
| HP | 6500 |
| ATK | 200 |
| DEF | 100 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Reflection:** Copies the deity's last ability at 50% power.  
**Ability — Shatter Mirror:** 240 ATK. If it copies an ability, next attack deals +100%.  
**Behavior:** Unpredictable. Mirrors deity's own kit back at them.  

### 014. Hollow Root-Fracture — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 230 |
| DEF | 120 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Root Sever:** 200 ATK. Disables deity's Passive for 3s.  
**Ability — Earth Crack:** 180 ATK AoE. Creates a fissure that slows movement.  
**Behavior:** Anti-passive specialist. Disables deity identity.  

### 015. Hollow Cinder-Chain Bailiff — Elite
| Stat | Value |
|------|-------|
| HP | 6800 |
| ATK | 220 |
| DEF | 130 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Chain Drag:** Pulls deity toward self. 100 ATK.  
**Ability — Cinder Burst:** 260 ATK AoE. Applies Burn (30 ATK/s for 3s).  
**Behavior:** Position disruptor. Pulls deity into danger zones.  

### 016. Hollow Throne Guard — Mini-Boss
| Stat | Value |
|------|-------|
| HP | 10000 |
| ATK | 320 |
| DEF | 160 |
| SPD | 4 |
| DMG TYPE | Void |
**Ability — Throne Slam:** 400 ATK AoE. 1s telegraph. Stuns 2s.  
**Ability — Guard Stance:** Takes 50% less damage for 4s. Cannot move during this.  
**Ability — Void Roar:** 250 ATK. Pushes all enemies back 6m.  
**Phase 2 (50% HP):** ATK +30%. Gains "Unrelenting" — ignores CC immunity.  

### 017. Hollow Judgment-Scale — Mini-Boss
| Stat | Value |
|------|-------|
| HP | 9500 |
| ATK | 300 |
| DEF | 150 |
| SPD | 5 |
| DMG TYPE | Void |
**Ability — Weigh Soul:** Deals damage equal to deity's current DE x 2.  
**Ability — Hollow Verdict:** If deity's DE > 50%, Stuns for 2s. If DE < 50%, drains 50 DE.  
**Ability — Scale Tilt:** 280 ATK. Slows 50% for 3s.  
**Phase 2 (50% HP):** "Weigh Soul" triggers every 5s automatically.  

### 018. Hollow World-Wound Behemoth — World Boss (See World Boss Section)

---

## ATEN RA TROOPS (019-021)
**Faction Damage Type:** Solar  
**Visual:** Electrum-gold skin, pharaonic armor, solar aura, colossal environment scale  

### 019. Aten Ra Khepri Scarab — Trash
| Stat | Value |
|------|-------|
| HP | 2500 |
| ATK | 110 |
| DEF | 40 |
| SPD | 6 |
| DMG TYPE | Solar |
**Ability — Scarab Charge:** Dashes at deity. 100 ATK. Knockback.  
**Behavior:** Charges in packs. Fast but fragile. Easy to stagger.  

### 020. Aten Ra Uraeus Serpent — Elite
| Stat | Value |
|------|-------|
| HP | 6000 |
| ATK | 200 |
| DEF | 90 |
| SPD | 5 |
| DMG TYPE | Solar |
**Ability — Venom Spit:** Ranged. 150 ATK. Applies Poison (40 ATK/s for 4s).  
**Ability — Coil Crush:** 250 ATK. Wraps and Stuns for 1.5s.  
**Behavior:** Ranged harasser. Applies pressure with poison.  

### 021. Aten Ra Solar Judge — Elite
| Stat | Value |
|------|-------|
| HP | 6500 |
| ATK | 230 |
| DEF | 110 |
| SPD | 4 |
| DMG TYPE | Solar |
**Ability — Solar Beam:** Long-range beam. 200 ATK. Pierces.  
**Ability — Ma'at Weighing:** If deity's HP > 70%, deals +50% damage.  
**Behavior:** Long-range threat. Punishes high-HP play. Encourages aggressive play.  

---

## ASGARDIAN TROOPS (022-024)
**Faction Damage Type:** Storm  
**Visual:** Warm gunmetal skin, frost-iron armor, blue-white lightning  

### 022. Asgardian Rime Raider — Trash
| Stat | Value |
|------|-------|
| HP | 2800 |
| ATK | 120 |
| DEF | 50 |
| SPD | 5 |
| DMG TYPE | Frost |
**Ability — Frost Axe:** 100 ATK. Applies Slow 30% for 2s.  
**Behavior:** Swarms in shield-wall formation. Slows deity to set up heavier units.  

### 023. Asgardian Thunder-Bridge Jotun — Elite
| Stat | Value |
|------|-------|
| HP | 7500 |
| ATK | 260 |
| DEF | 130 |
| SPD | 3 |
| DMG TYPE | Storm |
**Ability — Frost Club:** 300 ATK. 1.5s telegraph. Knockback.  
**Ability — Storm Surge:** 180 ATK AoE. Stuns for 1s.  
**Behavior:** Heavy bruiser. Slow, hits hard, telegraphs everything.  

### 024. Asgardian Valkyr Storm-Caller — Elite
| Stat | Value |
|------|-------|
| HP | 5500 |
| ATK | 220 |
| DEF | 80 |
| SPD | 6 |
| DMG TYPE | Storm |
**Ability — Storm Call:** 160 ATK AoE at target location. 2s delay.  
**Ability — Wind Step:** Teleports short distance. Leaves storm trail (50 ATK/s).  
**Behavior:** Mobile caster. Teleports and drops AoE. Hard to pin down.  

---

## OLYMPIAN TROOPS (025-027)
**Faction Damage Type:** Divine  
**Visual:** Living marble skin, bronze armor, gold-blue skyfire  

### 025. Olympian Laurel Hoplon Guard — Trash
| Stat | Value |
|------|-------|
| HP | 3000 |
| ATK | 100 |
| DEF | 80 |
| SPD | 3 |
| DMG TYPE | Divine |
**Ability — Shield Wall:** Reduces frontal damage by 60%. Cannot attack while active.  
**Ability — Shield Bash:** 120 ATK. Staggers for 1s.  
**Behavior:** Defensive wall. Must be flanked or staggered. Blocks frontal assault.  

### 026. Olympian Oracle Glass Archer — Elite
| Stat | Value |
|------|-------|
| HP | 5000 |
| ATK | 240 |
| DEF | 70 |
| SPD | 5 |
| DMG TYPE | Divine |
**Ability — Oracle Shot:** 200 ATK. Pierces. Reveals deity for 3s (removes invisibility/stealth).  
**Ability — Foresight:** Dodges next attack. 15s cooldown.  
**Behavior:** Long-range sniper. Anti-stealth. Dodge makes it slippery.  

### 027. Olympian Bronze Chimera Trialbeast — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 250 |
| DEF | 120 |
| SPD | 5 |
| DMG TYPE | Divine |
**Ability — Ram Charge:** Charges in a line. 280 ATK. Knockback.  
**Ability — Lion's Roar:** 180 ATK AoE. Feared for 1.5s.  
**Ability — Serpent Tail:** 150 ATK. Applies Poison (30 ATK/s for 3s).  
**Behavior:** Multi-attack bruiser. Three attack types. No telegraph on tail.  

---

## KAMI TROOPS (028-030)
**Faction Damage Type:** Spirit  
**Visual:** Living lacquer skin, shrine-forged armor, foxfire blue  

### 028. Kami Torii Foxfire Runner — Trash
| Stat | Value |
|------|-------|
| HP | 2200 |
| ATK | 90 |
| DEF | 30 |
| SPD | 7 |
| DMG TYPE | Spirit |
**Ability — Foxfire Dash:** Dashes through deity. 80 ATK. Leaves foxfire trail (40 ATK/s for 2s).  
**Ability — Phase Out:** Becomes intangible for 1s. 6s cooldown.  
**Behavior:** Extremely fast. Hits and runs. Phase makes it hard to hit.  

### 029. Kami Shimenawa Binder — Elite
| Stat | Value |
|------|-------|
| HP | 6000 |
| ATK | 200 |
| DEF | 100 |
| SPD | 4 |
| DMG TYPE | Spirit |
**Ability — Sacred Rope:** Roots deity for 2.5s. 120 ATK.  
**Ability — Purification Seal**: Disables one random ability for 3s.  
**Behavior:** Controller. Binds and seals abilities. Sets up for other troops.  

### 030. Kami Oni Iron-Fracture — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 260 |
| DEF | 140 |
| SPD | 4 |
| DMG TYPE | Spirit |
**Ability — Iron Club:** 300 ATK. 1s telegraph. Staggers.  
**Ability — Oni Roar:** 150 ATK AoE. Reduces deity DEF by 30% for 4s.  
**Behavior:** Heavy hitter. Breaks defenses. Telegraphs club but roar is instant.  

---

## TUATHA TROOPS (031-033)
**Faction Damage Type:** Nature  
**Visual:** Living wood-bark skin, druidic bone/antler armor, silver-green mist  

### 031. Tuatha Thornbound Strangler — Trash
| Stat | Value |
|------|-------|
| HP | 2400 |
| ATK | 100 |
| DEF | 40 |
| SPD | 5 |
| DMG TYPE | Nature |
**Ability — Thorn Grasp:** 90 ATK. Roots for 1.5s.  
**Ability — Bramble Crawl**: Moves through walls/obstacles.  
**Behavior:** Ambushers. Emerge from terrain. Root and flee.  

### 032. Tuatha Ancient Root Ogre — Elite
| Stat | Value |
|------|-------|
| HP | 7500 |
| ATK | 250 |
| DEF | 130 |
| SPD | 3 |
| DMG TYPE | Nature |
**Ability — Root Fist:** 280 ATK. 1s telegraph. Knockback.  
**Ability — Earth Drink**: Heals 1000 HP over 3s. Cannot move during heal.  
**Behavior:** Tanky bruiser. Heals when left alone. Must pressure.  

### 033. Tuatha Druidic Seer — Elite
| Stat | Value |
|------|-------|
| HP | 5500 |
| ATK | 210 |
| DEF | 80 |
| SPD | 5 |
| DMG TYPE | Nature |
**Ability — Geas Curse:** 150 ATK. Disables deity's Ultimate for 6s.  
**Ability — Mist Veil**: Creates fog. Deity loses vision beyond 4m for 4s.  
**Behavior:** Support caster. Disables ultimates. Blinds. Priority target.  

---

## EMPYREAN TROOPS (034-036)
**Faction Damage Type:** Divine  
**Visual:** Living light skin, ivory-gold plate, helmets containing radiance  

### 034. Empyrean Choir Vanguard — Trash
| Stat | Value |
|------|-------|
| HP | 2600 |
| ATK | 100 |
| DEF | 60 |
| SPD | 4 |
| DMG TYPE | Divine |
**Ability — Radiant Strike:** 90 ATK. Marks deity for +10% damage taken (stacks 3x).  
**Ability — Choir Resonance**: When 3+ Vanguards are near, all gain +30% ATK.  
**Behavior:** Formation fighters. Stronger in groups. Must split them up.  

### 035. Empyrean Light-Clad Sentinel — Elite
| Stat | Value |
|------|-------|
| HP | 6500 |
| ATK | 230 |
| DEF | 120 |
| SPD | 5 |
| DMG TYPE | Divine |
**Ability — Judgment Beam:** 200 ATK. Line pierce. Silences for 1.5s.  
**Ability — Containment Field**: Creates a barrier. Deity cannot enter for 3s.  
**Behavior:** Zone controller. Blocks paths. Silences. Keeps deity at range.  

### 036. Empyrean Seraphic Iron-Clad — Elite
| Stat | Value |
|------|-------|
| HP | 7000 |
| ATK | 250 |
| DEF | 150 |
| SPD | 4 |
| DMG TYPE | Divine |
**Ability — Angelic Cleave:** 280 ATK. Wide arc. Staggers.  
**Ability — Divine Aegis**: Blocks next 2 hits. Reflects 150 ATK Divine.  
**Ability — Radiant Burst**: 200 ATK AoE. Blinds for 2s.  
**Behavior:** Heavy paladin. Tanky, hits hard, shields. All-rounder threat.  

---

## INFERNAL DOMINION TROOPS (037-039)
**Faction Damage Type:** Infernal  
**Visual:** Living ash skin, black iron plate, ember-red anti-radiance, bare demon feet  

### 037. Infernal Cinder-Chain Bailiff — Elite
| Stat | Value |
|------|-------|
| HP | 6500 |
| ATK | 230 |
| DEF | 120 |
| SPD | 4 |
| DMG TYPE | Infernal |
**Ability — Chain Lash:** 220 ATK. Pulls deity 3m closer.  
**Ability — Cinder Step**: Leaves ember trail. 60 ATK/s Infernal. 4s duration.  
**Ability — Debt Collector**: If deity has a buff, deals +50% damage.  
**Behavior:** Position disruptor + anti-buff. Pulls and punishes buffs.  

### 038. Infernal Brass Hellhound Packfiend — Trash (appears in packs of 2-3)
| Stat | Value |
|------|-------|
| HP | 2000 |
| ATK | 130 |
| DEF | 40 |
| SPD | 7 |
| DMG TYPE | Infernal |
**Ability — Pack Bite:** 100 ATK. If 2+ Hellhounds near target, each bite deals +50%.  
**Ability — Ember Howl**: Buffs all nearby Hellhounds with +30% ATK and +2 SPD for 4s.  
**Ability — Searing Dash**: Dashes through deity. 80 ATK. Applies Burn (20 ATK/s for 2s).  
**Behavior:** Fast pack hunters. Dangerous in groups. Must isolate and kill quickly.  

### 039. Infernal Black Iron Executor — Elite
| Stat | Value |
|------|-------|
| HP | 7500 |
| ATK | 270 |
| DEF | 140 |
| SPD | 3 |
| DMG TYPE | Infernal |
**Ability — Execution Axe:** 320 ATK. 1.5s telegraph. Ignores 50% DEF.  
**Ability — Furnace Burst**: 200 ATK AoE. Burns ground for 4s (60 ATK/s).  
**Ability — Sanction Mark**: Marks deity. If deity's HP drops below 30% while marked, executes.  
**Behavior:** Heavy executioner. Telegraphs axe but sanction mark creates lethal pressure. Must clear mark.  

---

# SECTION 3: WORLD BOSSES (4)

### WB-1. Leviathan of the First Flood
**Faction:** Pre-Gate Mythic Deep  
**Threat Tier:** World Boss  
**Combat Role:** WORLD BOSS  

| Stat | Value |
|------|-------|
| HP | 200000 |
| ATK | 800 |
| DEF | 300 (35% reduction) |
| SPD | 4 |
| DE | 500 |
| AREA | 10 |
| DMG TYPE | Storm / Frost |

**Phase 1 (100-66% HP):**  
- **Tidal Surge:** Floods the arena. Rising water deals 100 ATK/s to deity. Lasts 8s.  
- **Storm Body:** Lightning discharges from body. 250 ATK to all within 6m every 3s.  
- **Chain Sweep:** Swings chains across arena. 300 ATK. Knockback. Telegraphed.  

**Phase 2 (66-33% HP):**  
- **First Flood:** Arena fully submerged. Deity must stand on floating debris. Water deals 150 ATK/s.  
- **Leviathan Breath:** Massive water beam. 500 ATK. Sweeps across arena. 2s telegraph.  
- **Drown:** Pulls deity underwater. 200 ATK/s. Must mash to escape.  

**Phase 3 (33-0% HP):**  
- **Primordial Wrath:** ATK +50%. All attacks gain Frost damage (Freeze on hit, 1s).  
- **Moonlit Eyes:** Fires moon-beams from eyes. 400 ATK each. Tracks deity.  
- **Tidal Crown:** Drops massive ice chunks from horns. 600 ATK AoE. Random locations.  
- **Enrage Timer (90s):** If not killed in 90s after Phase 3 starts, auto-wipe.  

---

### WB-2. Hollow World-Wound Behemoth
**Faction:** Hollow  
**Threat Tier:** World Boss  
**Combat Role:** WORLD BOSS  

| Stat | Value |
|------|-------|
| HP | 180000 |
| ATK | 750 |
| DEF | 250 (28% reduction) |
| SPD | 3 |
| DE | 400 |
| AREA | 10 |
| DMG TYPE | Void |

**Phase 1 (100-50% HP):**  
- **Void Body:** Passive. Takes 50% less damage from non-Void sources. Weak to Divine.  
- **Wound Pulse:** Every 5s, releases a void wave. 200 ATK. Applies Hollowed for 4s.  
- **Gate Stone Fragment Throw:** Hurls a fragment. 350 ATK. Creates a void zone (100 ATK/s).  

**Phase 2 (50-0% HP):**  
- **World-Wound:** Arena splits. Half becomes void zone (200 ATK/s). Deity must fight on safe half.  
- **Absorb Identity:** Copies deity's Passive. Uses it against them for 10s.  
- **Anti-Light Cascade**: 500 ATK. Darkness fills arena. Deity vision reduced to 3m for 5s.  
- **Fracture Storm**: White fracture cracks spread across arena. 150 ATK/s to anyone standing on them.  
- **Enrage (120s):** All damage +100%. Void zones expand every 10s.  

---

### WB-3. The Gate Counter
**Faction:** Pre-Gate (Unaligned)  
**Threat Tier:** World Boss  
**Combat Role:** WORLD BOSS  

| Stat | Value |
|------|-------|
| HP | 250000 |
| ATK | 700 |
| DEF | 400 (45% reduction) |
| SPD | 2 |
| DE | 300 |
| AREA | 10 |
| DMG TYPE | Divine |

**Phase 1 (100-60% HP):**  
- **Living Stone:** Passive. Regenerates 500 HP/s. Cannot be CC'd. Immune to knockback.  
- **Colossal Fist:** 400 ATK. 2s telegraph. Must dodge — cannot block or mitigate.  
- **Gate Slam:** Slams the Gate frame. 300 ATK AoE. Stuns 2s. Earthquake effect.  
- **Root Prison:** Roots grow from the ground. Roots deity for 3s. Must break free.  

**Phase 2 (60-30% HP):**  
- **Shattered Weapon:** Picks up broken weapon. Gains 500 ATK melee range attacks.  
- **Amber Pulse:** Fading amber-gold light radiates. 250 ATK. Heals Guardian for 2000 HP.  
- **Gate Memory:** Gate activates briefly. Summons 3 Trash enemies from the deity's own faction.  

**Phase 3 (30-0% HP):**  
- **Fading Light:** Regen increases to 1000 HP/s. All attacks gain +50% damage.  
- **Final Stand:** Guardian fuses fully with Gate. Arena shrinks. Safe zone contracts every 10s.  
- **Amber Cataclysm:** 800 ATK massive AoE. 3s telegraph. Must be outside radius.  
- **Enrage (180s):** If not killed, Guardian fully fuses. Arena becomes void. Auto-wipe.  

---

### WB-4. The Forgotten Giant
**Faction:** Forgotten  
**Threat Tier:** World Boss  
**Combat Role:** WORLD BOSS  

| Stat | Value |
|------|-------|
| HP | 220000 |
| ATK | 650 |
| DEF | 200 (22% reduction) |
| SPD | 3 |
| DE | 400 |
| AREA | 10 |
| DMG TYPE | Frost / Void |

**Phase 1 (100-70% HP):**  
- **Forgetting Aura:** Passive. Deity loses 1 random ability for 5s every 15s. Cannot use it.  
- **Frost Step:** Every step creates frost zones. 80 ATK/s. Zones persist for entire fight.  
- **Shattered Spear Thrust:** 350 ATK. Long-range pierce. 1.5s telegraph.  

**Phase 2 (70-35% HP):**  
- **Erasure:** Half the arena starts "forgetting" — textures fade, terrain disappears. Standing in erased zones deals 200 ATK/s Void.  
- **Memory Loss:** Deity's Passive is disabled for this entire phase.  
- **Frost Beard Release:** Releases frost from beard. 400 ATK AoE cone. Freezes for 2s.  
- **Forgotten Name:** Giant calls out a name nobody remembers. 300 ATK. Silences for 3s.  

**Phase 3 (35-0% HP):**  
- **Half-Erased:** Giant's right side fully dissolves. Becomes intangible from the right. Can only be damaged from the left side.  
- **Walking Erasure:** Giant walks slowly across arena. Everything behind it is erased (permanent void zones).  
- **Final Memory:** Giant remembers something. 600 ATK massive AoE. 4s telegraph. Screen goes white briefly.  
- **Frost Cataclysm:** 500 ATK. All frost zones on arena explode simultaneously.  
- **Enrage (150s):** Giant fully forgets itself. Dissolves. Arena becomes void. Auto-wipe.  

---

## END OF STAT SHEETS
Total: 28 Deities + 39 Creatures + 4 World Bosses = 71 Assets
