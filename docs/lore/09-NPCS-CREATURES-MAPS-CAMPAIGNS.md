# NPCs, Creatures, Maps, and Campaign Canon


> **Avatar System Note**: Players control Avatars — divine projections of their chosen deity. Avatars carry the deity's power and faith triggers into combat. The deity itself remains protected in its domain. Progression is driven by Belief and Influence. See `docs/lore/04-DEITIES.md` for the full Avatar System specification.


This document expands Mythos Gates: Ascension beyond the deity roster without breaking the Deity-only playable canon.

## Director Ruling

- The Lore Director authorizes NPCs as narrative, briefing, Codex, antagonist, or campaign support figures only.
- NPCs are not playable non-deity entries.
- Creatures are enemies, hazards, or world boss threats. They are not deities and do not belong to playable chosen deity deployments.
- Maps are tactical or campaign locations tied to Gates, Realms, hazards, and objectives.
- Campaigns organize chapters, maps, NPCs, and creatures without allowing unrestricted faction mixing.

## New Canon Systems

### Non-Playable Characters

NPCs support story, tutorials, Codex unlocks, faction identity, and antagonist pressure. They do not enter the one-active-deity chosen deity.

### Creatures

Creatures represent Hollow leakage, Gateborn catastrophes, Forgotten remnants, Beast Realm predators, and Ancient World Bosses.

### Maps

Maps define tactical spaces, hazards, objectives, and Realm-specific environmental identity.

### Campaigns

Campaigns connect chapters into Director-approved progression arcs.

## Current Campaign Spine

1. **The First Reopening** — playable scaffold and onboarding campaign.
2. **Trial of the Celestial Heights** — Olympian and Asgardian trial/oath conflict.
3. **Roots Below the Gate** — Avalora living terrain and restoration campaign.
4. **Cinders of Dominion** — Infernal bargains, execution sigils, and power politics.

## Safety Rule

No entry in this system may resurrect removed lore, introduce sci-fi, add extradimensional beings, add futuristic craft, or convert NPCs into playable non-deity entries without explicit Creator approval.

## Expanded Campaign Lore Coverage

Each current campaign now has a dedicated lore file that ties its maps, NPCs, threats, and Realm philosophy back to the canon foundation. Dialogue drafts must follow the Campaign Dialogue Bible: restrained, mythic, strategic, and never modern-cheesy.

Current lore-covered campaigns:

1. The First Reopening
2. Trial of the Celestial Heights
3. Roots Below the Gate
4. Cinders of Dominion
5. Helio-Kemet Sun Realm
6. Kami Mirror Tempest
7. Empyrean Choir Vault
