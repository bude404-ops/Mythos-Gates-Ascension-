# Tuatha: The Root-Crowned Green Below
**Codex ID:** MG-REALM-005  
**Director:** Lore Director  
**Faction:** Tuatha  
**Gate:** The Thorn-Memory Gate  
**Status:** Canon Expanded
## Realm Thesis
Tuatha deities protect continuity. They know worlds die when stories lose roots, names, and obligations to the dead.
## Central Conflict
Their memory is power, but it can become vengeance when old wounds refuse to compost.
## Realm Atmosphere
The Root-Crowned Green Below is defined by ancient roots, living songs, moon pools, stone scripts, barrow dawns, and nature remembering every betrayal. Its visual language is emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist.
## Major Landmarks
- **The Moon-Grove Rootlabyrinth** — a campaign-facing location for Tuatha stories.
- **The Silver Pool** — a campaign-facing location for Tuatha stories.
- **The Thorn-Song Court** — a campaign-facing location for Tuatha stories.
- **The Barrow Dawn** — a campaign-facing location for Tuatha stories.
- **The Stone Script Hill** — a campaign-facing location for Tuatha stories.


> **Avatar System**: Players channel Avatars of their chosen deity — divine projections that carry the deity's power into combat. The deity remains protected in its domain. Progression via Belief and Influence. Avatar death = respawn at deity domain with 10-20% penalty. Solo combat only. God-scale in all realms.


## Realm Hazards
- Roots that bind liars.
- fae bargains hidden in birdsong.
- barrow lights that lure deities into old wars.

## Campaign Hooks
- Recover a stolen ancestral name.
- bargain with the dead without waking a forgotten army.
- stop root-memory from consuming a living city.

## Deity Codex Hooks
### Dagda Root-Crown (MG-DEITY-037)
- **Role:** Endurance
- **Lore Function:** Dagda Root-Crown embodies the Tuatha answer to the Ascension Collapse: defender shaped into Realm doctrine.
- **Chapter Hook:** When Dagda Root-Crown enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Dagda Root-Crown against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Brigid Emerald Oath (MG-DEITY-040)
- **Role:** Conduit
- **Lore Function:** Brigid Emerald Oath embodies the Tuatha answer to the Ascension Collapse: battery shaped into Realm doctrine.
- **Chapter Hook:** When Brigid Emerald Oath enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Brigid Emerald Oath against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Morrigan Moon-Grove (MG-DEITY-041)
- **Role:** Dominion
- **Lore Function:** Morrigan Moon-Grove embodies the Tuatha answer to the Ascension Collapse: controller shaped into Realm doctrine.
- **Chapter Hook:** When Morrigan Moon-Grove enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Morrigan Moon-Grove against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Lugh Granite Bough (MG-DEITY-040)
- **Role:** Fracture
- **Lore Function:** Lugh Granite Bough embodies the Tuatha answer to the Ascension Collapse: breaker shaped into Realm doctrine.
- **Chapter Hook:** When Lugh Granite Bough enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Lugh Granite Bough against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Nimue Silver Pool (MG-DEITY-041)
- **Role:** Disruptor
- **Lore Function:** Nimue Silver Pool embodies the Tuatha answer to the Ascension Collapse: disruptor shaped into Realm doctrine.
- **Chapter Hook:** When Nimue Silver Pool enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Nimue Silver Pool against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Cernan Wild Vale (MG-DEITY-042)
- **Role:** Sustain
- **Lore Function:** Cernan Wild Vale embodies the Tuatha answer to the Ascension Collapse: sustain shaped into Realm doctrine.
- **Chapter Hook:** When Cernan Wild Vale enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Cernan Wild Vale against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Ailbhe Thorn-Song (MG-DEITY-041)
- **Role:** Artillery
- **Lore Function:** Ailbhe Thorn-Song embodies the Tuatha answer to the Ascension Collapse: artillery shaped into Realm doctrine.
- **Chapter Hook:** When Ailbhe Thorn-Song enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Ailbhe Thorn-Song against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Oghma Stone Script (MG-DEITY-042)
- **Role:** Counter
- **Lore Function:** Oghma Stone Script embodies the Tuatha answer to the Ascension Collapse: guardian shaped into Realm doctrine.
- **Chapter Hook:** When Oghma Stone Script enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Oghma Stone Script against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.
### Tara Dawn Barrow (MG-DEITY-037)
- **Role:** Shadow
- **Lore Function:** Tara Dawn Barrow embodies the Tuatha answer to the Ascension Collapse: assassin shaped into Realm doctrine.
- **Chapter Hook:** When Tara Dawn Barrow enters a chapter, the scene should test Tuatha's core tension: Their memory is power, but it can become vengeance when old wounds refuse to compost.
- **Visual Context:** Frame Tara Dawn Barrow against emerald forests, silver moon pools, Celtic knotwork, antler crowns, moss-covered megaliths, golden dawn mist while preserving their individual silhouette and role readability.

## Director Ruling
This chapter expands faction, Realm, and deity presentation only. It does not create playable non-deity entries, normal units, extradimensional factions, futuristic craft, or cross-faction strike-force rules.
