...
**THE TENDRIL SYSTEM (Sept 4 2026, BudE404: 'OK make tendrils wings' — dual-system CONFIRMED):** Wing anchors are always armored wing-root collars fused to the chest back-plate; out of the roots pours the deity's OWN ELEMENT as sweeping tendril-wings (Diablo-archangel silhouette, element-native). TENDRIL wings = casters, heralds, agile strikers (Sorvath: fire tendrils — FIRST tendril-wing deity, v4). PLATE wings = wardens and heaviest builds (Vharkar canon untouched, Morvain, Thevril). Every wing is armor-rooted; the blade is either poured element or forged plate. TENDRIL COUNT + SCALE LAW (BudE404, updated: '5 tendrils each side looked the best'): exactly 5 thick DISTINCT tendrils per side, one armored root per shoulder blade (both sides always), separated by clear gaps so the wing reads as fingers, NEVER a unified flowing mass — no cape, no mantle, no sheet of element. WEAPON COUNT LAW (BudE404, Sorvath v7 kill): exactly ONE arrow for archer deities — no second arrow, no quiver, no extra arrows; count-check every ranged roll. SCALE: each tendril root as thick as the deity's own torso, length ~2x body height, slow taper over the full sweep, outermost longest/highest — wings dwarf the figure, true Diablo archangel proportions. FLOW LAW (BudE404: 'need them to flow more down'): tendrils rise out of the armored roots, arc ONCE high above the shoulders, then FLOW DOWNWARD in long heavy cascading curves — fire poured from the roots, raining down behind the figure, tips trailing toward waist/knee level. Tall at the root arc, long and low at the descent — wings of flowing downward motion.
