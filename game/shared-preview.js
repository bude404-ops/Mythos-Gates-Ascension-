...
function battleHybrid(data, state) {
  ...
  const cells = Array.from({length:35},(_,i)=>{
    ...
    const gate = i === 3;
    return `<div class="tactical-cell ${gate?'gate-cell':''}">${gate?'<b>3D GATE</b>':playerIndex>=0?spriteToken(playerTitans[playerIndex],'player',playerIndex===0?'titan':'normal'):enemyIndex>=0?spriteToken(enemies[enemyIndex],'enemy',enemyIndex===2?'titan':'normal'):''}</div>`;
  }).join('');
  return `<div class="hybrid-label"><b>3D TACTICAL ENVIRONMENT + 2D SPRITES</b><span>Combatants remain 2D · environment supplies depth, terrain, lighting, and scale</span></div><section class="battle-hybrid"><div class="battle-camera">Mobile tactical camera · no terrain-hidden sprites</div><div class="tactical-grid">${cells}</div></section><div class="game-card"><h3>${esc(map?.name || 'Tactical Battlefield')}</h3><p>2D Titan and enemy sprites are grounded with contact shadows, selection rings, consistent scale, and grid positioning inside a 3D battlefield shell.</p><p class="ability">Gameplay system in development: live tactical engine will mount here when battle systems expand.</p>${state.devMode ? `<div class="dev-chip">component: BattleHybrid · sprite layer: 2D · environment layer: 3D</div>` : ''}</div>`;
}
