...
const currentSpace = s => s.terrain.spaces.find(sp=>key(sp.position)===key(s.deity.position));
const ARCHETYPE_BUDGETS = Object.freeze({
  ...
  ENEMY_DEITY:{ hp:1.8, damage:1.38, armor:1.35, resistance:1.35, threat:4.6, countPressure:0.35, mechanics:['stance mirror','divine phase'] },
...
const profileFor = e => {
  ...
  if(role === 'SWARMER') return { id:'HOLLOW_SWARMER', label:'Swarm Pressure', reactionType:'DODGE', preferredRange:1, movementBias:'close', telegraph:'Rushes isolated Deities and forces reaction spending.' };
  ...
  if(role === 'EXECUTIONER') return { id:'WOUNDED_EXECUTIONER', label:'Devour Weakness', reactionType:'DODGE', preferredRange:1, movementBias:'hunt', telegraph:'Tracks wounded Deities and escalates if ignored.' };
...
function chooseEnemyIntent(state, e){
  const dist = distance(e.position,state.deity.position);
  const profile = profileFor(e);
  if(dist > (e.range||profile.preferredRange)) return { type:'ADVANCE', label:'Advance', target:state.deity.id, telegraphed:true, reactionType:'COUNTER_CHARGE', description:`${e.name} closes distance.`, damage:0, pressure:0 };
  ...
  if(profile.id==='GATEBORN_BRUTE'){
    ...
    return { type, label:intentLabel(e,type), target: type==='OBJECTIVE_CRUSH'?'objective':state.deity.id, telegraphed:true, reactionType:type==='FRACTURE_ROAR'?'DODGE':'PARRY', description:type==='OBJECTIVE_CRUSH'?'The Colossus turns toward the seal objective; interrupt or finish the seal before the stomp lands.':type==='FRACTURE_ROAR'?'A fracture roar threatens Momentum and stance; dodge to preserve tempo.':'A crushing Gate Stomp is coming; parry to blunt the anchor hit.', damage:type==='FRACTURE_ROAR'?Math.max(1,Math.round(e.damage*.55)):e.damage+4, pressure:e.threatWeight||2, behaviorTag:type==='OBJECTIVE_CRUSH'?'OBJECTIVE_DENIAL':type==='FRACTURE_ROAR'?'MOMENTUM_BREAK':'ANCHOR_STOMP', counterplay:type==='OBJECTIVE_CRUSH'?'Progress vulnerable objectives after the Colossus acts or spend a high-impact turn to force reaction tempo.':type==='FRACTURE_ROAR'?'Hold Momentum above reaction cost before the roar.' :'Parry in Guardian stance to turn the hit into counter damage.' };
  }
  if(profile.id==='WOUNDED_EXECUTIONER') return { type:'DEVOUR_WEAKNESS', label:intentLabel(e,'DEVOUR_WEAKNESS'), target:state.deity.id, telegraphed:true, reactionType:'DODGE', description:'The creature hunts low health and marked weakness.', damage:e.damage + (state.deity.hp <= state.deity.maxHp*.45 ? 6 : 0), pressure:2 };
  return { type:'STRIKE', label:intentLabel(e,'STRIKE'), target:state.deity.id, telegraphed:true, reactionType:profile.reactionType, description:profile.telegraph, damage:e.damage, pressure:1 };
}
...
export function applyDeityAction(input,action){
  ...
  else if(action.type==='STANCE_SHIFT'){ if(!Object.values(STANCES).includes(action.stance)) throw new Error('Invalid stance'); if(action.stance===STANCES.ASCENDANT&&!s.deity.ascended) throw new Error('Ascendant stance requires Divine Ascension'); s.deity.stance=action.stance; log(s,'STANCE_SHIFT',{stance:action.stance}); }
  ...
  else throw new Error(`Unknown Deity action ${action.type}`);
  if(!living(s).length) s.phase=PHASES.OBJECTIVE; s.deity.actionsTakenThisRound.push(action.type); s.telemetry.actionTypeCounts[action.type]=(s.telemetry.actionTypeCounts[action.type]||0)+1; return s;
}

export function revealEnemyIntents(input){
  const s=clone(input); s.phase=PHASES.ENEMY_INTENT;
  for(const e of living(s)){ e.intent=chooseEnemyIntent(s,e); s.telemetry.enemyTelegraphs+=1; s.telemetry.enemyIntentCounts[e.intent.type]=(s.telemetry.enemyIntentCounts[e.intent.type]||0)+1; if(e.intent.behaviorTag) s.telemetry.enemyBehaviorTags[e.intent.behaviorTag]=(s.telemetry.enemyBehaviorTags[e.intent.behaviorTag]||0)+1; if(e.intent.counterplay && !s.telemetry.enemyCounterplay.includes(e.intent.counterplay)) s.telemetry.enemyCounterplay.push(e.intent.counterplay); log(s,'ENEMY_INTENT',{enemy:e.instanceId,intent:e.intent.type,label:e.intent.label,reactionType:e.intent.reactionType,pressure:e.intent.pressure,behaviorTag:e.intent.behaviorTag,counterplay:e.intent.counterplay,description:e.intent.description}); }
...
export function resolveEnemyPhase(input){
  ...
  if(e.intent.type==='ADVANCE'){
    const dx=Math.sign(s.deity.position.x-e.position.x); const dy=Math.sign(s.deity.position.y-e.position.y);
    const steps=Math.max(1,Math.min(e.movement||1,2));
    for(let i=0;i<steps;i++){ if(Math.abs(s.deity.position.x-e.position.x)>=Math.abs(s.deity.position.y-e.position.y)) e.position.x+=dx; else e.position.y+=dy; }
  ...
  if(e.intent.type==='FRACTURE_ROAR'){
    const loss=Math.min(s.resources.momentum||0,8); s.resources.momentum=clamp((s.resources.momentum||0)-loss); s.deity.status=[...new Set([...(s.deity.status||[]),'FRACTURE_ROAR'])]; log(s,'FRACTURE_ROAR',{enemy:e.instanceId,momentumLost:loss});
  }
...
export function applyReaction(input,choice='RESOLVE'){
  ...
  if(choice==='RESOLVE'){ const type=s.reactionWindow.type; if(type==='DODGE'){ gain(s,'momentum',12,'successful_dodge'); gain(s,'divinity',6,'successful_dodge'); s.telemetry.reactionSuccesses+=1; log(s,'REACTION_SUCCESS',{type,enemy:e.instanceId,prevented:e.damage}); } else { const dmg=Math.max(1,Math.round(s.deity.attack*.9)-Math.floor(e.armor/6)); e.hp=Math.max(0,e.hp-dmg); e.vulnerable=e.hp>0&&e.hp<=Math.ceil(e.maxHp*.35); s.telemetry.damageDealt+=dmg; gain(s,'momentum',14,'successful_counter_reaction'); gain(s,'divinity',8,'successful_counter_reaction'); s.telemetry.reactionSuccesses+=1; log(s,'REACTION_SUCCESS',{type,enemy:e.instanceId,counterDamage:dmg,enemyHp:e.hp}); } }
  else { const rawDamage=e.intent?.damage ?? e.damage; const mitigation=s.deity.stance===STANCES.GUARDIAN?Math.floor(s.deity.armor/3):Math.floor(s.deity.armor/5); const dmg=Math.max(1,rawDamage-mitigation); s.deity.hp=Math.max(0,s.deity.hp-dmg); s.telemetry.damageTaken+=dmg; s.telemetry.reactionDeclines+=1; log(s,'REACTION_DECLINED',{enemy:e.instanceId,intent:e.intent?.type,damage:dmg,deityHp:s.deity.hp}); }
  e.intent=null; s.reactionWindow=null; s.telemetry.reactionsResolved+=1; s.phase=s.deity.hp<=0?PHASES.DEFEAT:PHASES.ENEMY; return s;
}
export function applyTerrainTick(input){
  const s=clone(input); s.phase=PHASES.TERRAIN; const sp=currentSpace(s); if(sp?.hazard==='SOLAR_JUDGMENT'){ const dmg=s.deity.stance===STANCES.GUARDIAN?1:4; s.deity.hp=Math.max(0,s.deity.hp-dmg); s.telemetry.damageTaken+=dmg; s.telemetry.hazardDamage+=dmg; gain(s,'solarCharge',8,'solar_judgment_lane'); log(s,'TERRAIN_HAZARD',{hazard:sp.hazard,damage:dmg,deityHp:s.deity.hp}); } return s;
}
export function evaluateObjectives(input,objectiveEvent=null){
  ...
  if(s.deity.hp<=0) s.phase=PHASES.DEFEAT; else if(s.objectives.every(o=>o.status==='COMPLETE')) s.phase=PHASES.VICTORY; else { s.round+=1; s.telemetry.turns=s.round; s.deity.actionsTakenThisRound=[]; s.phase=PHASES.PLAYER; log(s,'ROUND_ADVANCE',{round:s.round}); } return s;
}
