...
export function createCommandHubRuntime(DATA, mount){
  ...
  function resolveRaidEconomy(player, attempt){
    ...
    const cappedReplayTokens=firstClear?tokenBase:Math.max(2,Math.min(6,tokenBase-weeklyCount));
    const raidTokens=Math.round((firstClear?tokenBase+8:cappedReplayTokens)*replayScalar);
    const signatureAlloy=Math.max(1,Math.round(materialBase*replayScalar));
