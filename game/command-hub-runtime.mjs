...
export function createCommandHubRuntime(DATA, mount){
  ...
  const titanById = id => deities().find(t => t.id === id);
  ...
  const FALLBACK_CREATURES = [
    ...
    { id:'TG-CREATURE-023', name:'Thunder-Bridge Jotun', combatRole:'BRUTE', abilities:['Storm Boulder','Oath Roar','Bridge Quake'], mapBehavior:'Controls elevation and pushes the Titan away from objectives.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:3, threatWeight:3 } },
    { id:'TG-CREATURE-024', name:'Valkyr Storm-Caller', combatRole:'GUARDIAN', abilities:['Gale Mark','Oath Mend','Raven Sight'], mapBehavior:'Buffs melee units and marks exposed Titan routes.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    ...
    // Kami (TG-FACTION-004)
    { id:'TG-CREATURE-028', name:'Kami Torii Foxfire Runner', combatRole:'HUNTER', abilities:['Foxfire Dash','Mirror Feint','Seal Nip'], mapBehavior:'Flanks the Titan and baits wasted AoE.', aiProfile:{ archetype:'HUNTER' }, stats:{ hp:88, damage:16, range:1, armor:5, resistance:5, movement:5, threatWeight:2 } },
    { id:'TG-CREATURE-029', name:'Mirror-Lake Onryo', combatRole:'DISRUPTOR', abilities:['Grudge Pull','Reflection Hex','Drowned Silence'], mapBehavior:'Pulls the Titan toward hazards and slows recovery.', aiProfile:{ archetype:'DISRUPTOR' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    ...
    { id:'TG-CREATURE-034', name:'Empyrean Radiant Censor', combatRole:'CONTROLLER', abilities:['Censure Beam','Aura Sight','Contradiction Brand'], mapBehavior:'Marks the Titan and supports disciplined ranged fire.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:5, resistance:5, movement:4, threatWeight:2 } },
    { id:'TG-CREATURE-035', name:'Throne-Spear Adjudicator', combatRole:'BRUTE', abilities:['Adjudicator Thrust','Radiant Interdict','Edict Step'], mapBehavior:'Uses reach and zone denial to pin Titan movement.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:4, threatWeight:3 } },
    ...
    { id:'TG-CREATURE-037', name:'Infernal Cinder-Chain Bailiff', combatRole:'CONTROLLER', abilities:['Chain Writ','Debt Drag','Cinder Lock'], mapBehavior:'Drags the Titan into bad positions and taxes movement.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    { id:'TG-CREATURE-038', name:'Brass Hellhound Packfiend', combatRole:'HUNTER', abilities:['Pack Lunge','Scent of Debt','Ash Snap'], mapBehavior:'Fast pressure unit that surrounds and exposes the Titan.', aiProfile:{ archetype:'HUNTER' }, stats:{ hp:88, damage:16, range:1, armor:5, resistance:5, movement:5, threatWeight:2 } },
  ...
  const AWAKENING_BEATS = [
    ...
    { step:9, id:'SECOND_TITAN_ENCOUNTER', lesson:'Second Titan encounter', missionBeat:'A second Titan appears through a story battlefield event and solves a problem differently.', playableSystem:'specialist contrast', milestone:'AWAKENING_BEAT_09' },
    { step:10, id:'TITAN_TRIAL_HANDOFF', lesson:'Divine Trial', missionBeat:'Temporarily pilot a second Titan with its mechanic, sample gear, and Divine Ascension.', playableSystem:'temporary trial', milestone:'AWAKENING_BEAT_10' },
    ...
    { step:12, id:'FACTION_TRIAL_HANDOFF', lesson:'Faction Trial', missionBeat:'A short faction trial teaches environment, combat philosophy, Codex, and titan identity.', playableSystem:'faction trial', milestone:'AWAKENING_BEAT_12' }
  ];
  ...
  function starterTitans(){ const rows=STARTER_TITAN_IDS.map(titanById).filter(Boolean); return rows.length?rows:(deities()[0]?[deities()[0]]:[]); }
  ...
  function trialTitans(){ const rows=TRIAL_TITAN_IDS.map(titanById).filter(Boolean); return rows.length?rows:deities().slice(1,4); }
  ...
  const AssetManager = {
    ...
    visibleArtReport(player=ensurePlayerState()){
      ...
      const selected=player.selectedTitans.map(id=>this.forEntity(id,'TITAN_PRESENTATION'));
  ...
  function canonicalDefaultPlayer(){
    ...
    if(!base.selectedTitans.length && deities()[0]) base.selectedTitans=[deities()[0].id];
  ...
  function buildBattle(){
    const player=ensurePlayerState(); const mission=missionById(player.campaignProgress.currentMissionId)||missions()[0]||{}; const titan=titanById(player.selectedTitans?.[0])||deities()[0];
    ...
    state.battle=createBattleState({ battleId:`TG-HUB-${mission.id||'FIRST-GATE'}`, missionId:mission.id||'TG-F01-C01-M01', titan, enemies:scaledEnemySet, objectives, scaling }); log('battle','Playable battle initialized',{ missionId:mission.id, titanId:titan?.id, scaling });
  }
  ...
  const api={
    ...
    chooseStarter(id){
      ...
      p.selectedTitans=[id]; const ob=ensureOnboarding(p); ob.starterTitanId=id; ob.starterChoiceReason=starterTitans().find(t=>t.id===id)?.role || 'Canon starter'; completeAwakeningBeat(p,1); if(!ob.milestones.includes('STARTER_TITAN_BOUND')) ob.milestones.push('STARTER_TITAN_BOUND'); p.notifications=deriveNotifications(p); savePlayerState(); AudioManager.play('titan_selected'); render();
    },
