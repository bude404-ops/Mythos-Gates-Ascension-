...
export function createCommandHubRuntime(DATA, mount){
  ...
  function titansScreen(){
    ...
    return shell(`${profileHeader(player)}<main class="mx-auto max-w-5xl space-y-4 px-3 pb-28 pt-3"><button onclick="TGHub.go('hub')" class="rounded-2xl bg-neutral-800 px-4 py-3 font-black">← Command Hub</button><header class="rounded-3xl border border-primary/40 bg-neutral-900 p-5"><p class="text-xs font-black uppercase tracking-[.28em] text-primary">Deity Roster</p><h1 class="text-4xl font-black">Canonical Deities</h1><p class="mt-2 text-neutral-300">Selection writes to PlayerState. Deity cards read canonical Deity IDs only.</p>${badge(`${player.selectedTitans.length} selected`,'ok')}</header><div class="grid gap-3 md:grid-cols-2">${rows}</div></main>`);
  }
