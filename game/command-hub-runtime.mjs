import { PHASES, STANCES, createBattleState, applyTitanAction, revealEnemyIntents, resolveEnemyPhase, applyReaction, applyTerrainTick, evaluateObjectives, autoAdvanceEnemyTurn, summarizeBattle } from './browser-battle-engine.mjs';
...
export function createCommandHubRuntime(DATA, mount){
  ...
  function buildBattle(){
    ...
    state.battle=createBattleState({ battleId:`TG-HUB-${mission.id||'FIRST-GATE'}`, missionId:mission.id||'TG-F01-C01-M01', titan, enemies:enemySet, objectives }); log('battle','Playable battle initialized',{ missionId:mission.id, titan:titan?.id });
  }
