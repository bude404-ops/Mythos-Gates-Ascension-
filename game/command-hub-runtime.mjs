...
export function createCommandHubRuntime(DATA, mount){
  ...
  const FALLBACK_CREATURES = [
    ...
    { id:'TG-CREATURE-034', name:'Empyrean Radiant Censor', combatRole:'CONTROLLER', abilities:['Censure Beam','Aura Sight','Contradiction Brand'], mapBehavior:'Marks the Deity and supports disciplined ranged fire.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:5, resistance:5, movement:4, threatWeight:2 } },
  ...
  function deriveNotifications(player){
    ...
    for(const id of player.selectedDeities||[]){ const t=deityById(id); if(t && (t.abilityDetails||[]).some(a=>a.unlockLevel<=player.level+1 && a.unlockLevel>player.level)) notes.push({ id:`TG-NOTE-${id}-UPGRADE`, type:'deity', label:'Deity upgrade soon', route:'deitys', entityId:id }); }
    return notes;
  }
...
=======
    ...
    return { problemTags, advantageRoles, recommendedTitans, favoredNotRequired:p.favoredNotRequired!==false, ownershipLock:p.ownershipLock===true, rule:p.rule||'Recommended advantages only — any valid active deity can attempt the mission.' };
>>>>>>> 919bdc51 (Mythos Gates: Ascension — Full repo migration)
  ...
  function createRaidAttempt(player,tier='RAID_NORMAL'){
    ...
    const attempt={ id:`TG-RAID-ATTEMPT-${Date.now()}`, raidId:'TG-RAID-001', bossId:boss.id, name:boss.name, tier, activeDeityId:player.selectedDeities?.[0], stageIndex:0, status:'ACTIVE', score:0, damageTaken:0, turns:0, resolvedStages:[], modifiers:['Ruptured Gate Lanes','Reaction Discipline'], startedAt:new Date().toISOString(), stages };
  ...
  function resolveTrialAttempt(player, attempt, approach='BALANCED'){
    ...
    const cache={ id:`TG-REWARD-${attempt.trialId}-${attempt.deityId}-${Date.now()}`, missionId:attempt.trialId, title:`${attempt.deityName} Trial Cache`, firstClear:firstShowcase, missionType:'DEITY_TRIAL', status:'PENDING', summary:`${quality}-grade temporary loadout showcase. Trial Favor persists; borrowed gear does not.`, grants:{ gateShards:shardGrant, ascensionEmber:0, deityRelics:firstShowcase?1:0, raidTokens:0, signatureAlloy:0, masterySeals:0, trialFavor:favor, xp }, trialSummary:completion, createdAt:new Date().toISOString() };
...
=======
    return `<main class="min-h-screen overflow-hidden bg-neutral-950 text-neutral-50"><section class="relative flex min-h-screen flex-col items-center justify-end p-5 pb-14 text-center"><div class="absolute inset-0 bg-gradient-to-b from-neutral-950 via-primary/10 to-neutral-950"></div><div class="absolute top-24 h-72 w-48 rounded-t-full border-4 border-primary/40 bg-neutral-900/60 shadow-dark-outline"></div><div class="absolute top-8 h-96 w-96 rounded-full bg-primary/10 blur-3xl"></div><div class="relative z-10 mb-12"><p class="text-xs font-black uppercase tracking-[.32em] text-primary">${first?'The Gate Awakens':'Gate Link Restored'}</p><h1 class="mt-2 text-5xl font-black leading-none">Mythos Gates:<br/>Ascension</h1><p class="mx-auto mt-4 max-w-sm text-neutral-300">${first?'A massive Gate opens in the dark. Divine energy stirs. Your first Titan answers.':'Returning player state detected. The Command Hub is ready.'}</p><button onclick="TGHub.enterHub()" class="mt-8 rounded-3xl bg-primary px-8 py-5 text-xl font-black text-white">TAP TO ENTER</button></div></section></main>`;
>>>>>>> 919bdc51 (Mythos Gates: Ascension — Full repo migration)
  }
...
=======
    return `<main class="min-h-screen overflow-auto bg-neutral-950 p-4 text-neutral-50"><section class="mx-auto max-w-5xl space-y-4"><header class="rounded-[2rem] border border-primary/40 bg-neutral-900 p-5"><p class="text-xs font-black uppercase tracking-[.3em] text-primary">Awakening Protocol · TG-DEV-026</p><h1 class="mt-2 text-5xl font-black leading-none">Choose the Titan that answers first.</h1><p class="mt-3 max-w-2xl text-neutral-300">Aten Ra opens the first safe Gate. You choose one active deity for the Awakening path — no full roster flood, no forced purchase pressure.</p><div class="mt-4 grid gap-2 sm:grid-cols-4"><div class="rounded-2xl bg-neutral-950 p-3"><p class="text-xs text-neutral-500">Starter</p><b>${esc(selected||'Unbound')}</b></div><div class="rounded-2xl bg-neutral-950 p-3"><p class="text-xs text-neutral-500">Beats</p><b>${esc(progress.done)}/${esc(progress.total)}</b></div><div class="rounded-2xl bg-neutral-950 p-3"><p class="text-xs text-neutral-500">Roster</p><b>${onboarding.fullRosterHidden?'Hidden':'Unlocked'}</b></div><div class="rounded-2xl bg-neutral-950 p-3"><p class="text-xs text-neutral-500">Policy</p><b>No P2W</b></div></div><div class="mt-4 h-2 rounded-full bg-neutral-800"><div class="h-full rounded-full bg-primary" style="width:${progress.percent}%"></div></div>${selected?badge('Starter bound','ok'):badge('Awaiting choice','bad')}</header><section class="grid gap-3 md:grid-cols-3">${cards}</section><section class="grid gap-3 md:grid-cols-[.9fr_1.1fr]"><article class="rounded-3xl border border-neutral-800 bg-neutral-900 p-4"><h2 class="text-2xl font-black">Awakening Beats</h2><ul class="mt-3 max-h-[520px] space-y-2 overflow-auto pr-1">${beats}</ul></article><article class="rounded-3xl border border-neutral-800 bg-neutral-900 p-4"><h2 class="text-2xl font-black">First Gate Handoff</h2><p class="mt-2 text-neutral-300">After binding, the Command Hub opens directly into the first solo mission. The 12-beat path now tracks tutorial, trial, and faction-discovery milestones before the roster opens.</p><button ${selected?'':'disabled'} onclick="TGHub.advanceAwakeningBeat()" class="mt-5 w-full rounded-3xl ${selected?'bg-neutral-800 text-white':'bg-neutral-800 text-neutral-500'} px-5 py-4 text-lg font-black">ADVANCE CURRENT BEAT</button><button ${selected?'':'disabled'} onclick="TGHub.finishAwakening()" class="mt-3 w-full rounded-3xl ${selected?'bg-primary text-white':'bg-neutral-800 text-neutral-500'} px-5 py-5 text-xl font-black">BEGIN AWAKENING MISSION</button></article></section></section></main>`;
>>>>>>> 919bdc51 (Mythos Gates: Ascension — Full repo migration)
  }
  function boot(){
...
=======
  function titansScreen(){
    ...
    return shell(`${profileHeader(player)}<main class="mx-auto max-w-5xl space-y-4 px-3 pb-28 pt-3"><button onclick="TGHub.go('hub')" class="rounded-2xl bg-neutral-800 px-4 py-3 font-black">← Command Hub</button><header class="rounded-3xl border border-primary/40 bg-neutral-900 p-5"><p class="text-xs font-black uppercase tracking-[.28em] text-primary">Deity Roster</p><h1 class="text-4xl font-black">Canonical Titans</h1><p class="mt-2 text-neutral-300">Selection writes to PlayerState. Titan cards read canonical Titan IDs only.</p>${badge(`${player.selectedTitans.length} selected`,'ok')}</header><div class="grid gap-3 md:grid-cols-2">${rows}</div></main>`);
>>>>>>> 919bdc51 (Mythos Gates: Ascension — Full repo migration)
  }
