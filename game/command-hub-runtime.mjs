...
export function createCommandHubRuntime(DATA, mount){
  ...
  const state = { route:'boot', bootIndex:0, startupMode:'firstLaunch', player:null, selectedTab:'battle', focusDeityId:null, battle:null, logs:[], lastError:null, perf:{ renderCount:0, partialResourceUpdates:0, routeRenderCount:{}, lastRenderMs:0, maxRenderMs:0, lastRoute:null, budgetMs:32, resourceOnlyBudgetMs:8, assetPreloadCount:0 } };
  const factions = () => DATA.factions || [];
  const deities = () => DATA.deitys || [];
  ...
  const FALLBACK_CREATURES = [
    { id:'MG-CREATURE-001', name:'Hollow Wretch', combatRole:'SWARMER', abilities:['Scratch of Forgetting','Swarm Pressure'], mapBehavior:'Rushes isolated Deities and punishes careless positioning.', aiProfile:{ archetype:'SWARMER' }, stats:{ hp:45, damage:8, range:1, armor:2, resistance:2, movement:4, threatWeight:1 } },
    { id:'MG-CREATURE-002', name:'Gateborn Colossus', combatRole:'BRUTE', abilities:['Gate Stomp','Fracture Roar','Objective Crush'], mapBehavior:'Controls central objectives and forces Deities to split pressure.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:131, damage:17, range:1, armor:11, resistance:5, movement:3, threatWeight:2 } },
    // Cross-faction enemies (019-039): 3 per faction for future missions
    // Aten Ra (MG-FACTION-001)
    { id:'MG-CREATURE-019', name:'Aten Ra Sun-Seal Vigilant', combatRole:'CONTROLLER', abilities:['Aten Ray Lattice','Ma\u2019at Measure Bind','Solar Lane Verdict'], mapBehavior:'Locks lanes and punishes Deities who ignore objective geometry.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:126, damage:22, range:3, armor:7, resistance:8, movement:4, threatWeight:3 } },
    { id:'MG-CREATURE-020', name:'Khepri Scarab Dawn Engine', combatRole:'BRUTE', abilities:['Dawn Roll','Wingflare Guard','Silt-Glass Crush'], mapBehavior:'Advances slowly, guards Gates, and forces AoE decisions.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:88, damage:16, range:1, armor:7, resistance:5, movement:3, threatWeight:2 } },
    { id:'MG-CREATURE-021', name:'Uraeus Counter-Edict Serpent', combatRole:'DISRUPTOR', abilities:['Counter-Edict Spit','Blinding Crown','Law Venom'], mapBehavior:'Applies accuracy and cooldown pressure near lanes.', aiProfile:{ archetype:'DISRUPTOR' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    // Asgardian (MG-FACTION-002)
    { id:'MG-CREATURE-022', name:'Storm-Oath Rime Raider', combatRole:'BRUTE', abilities:['Rime Axe Hook','Bridge-Oath Charge','Shieldwall Break'], mapBehavior:'Closes distance and uses chokepoints to force repositioning.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:88, damage:16, range:1, armor:7, resistance:5, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-023', name:'Thunder-Bridge Jotun', combatRole:'BRUTE', abilities:['Storm Boulder','Oath Roar','Bridge Quake'], mapBehavior:'Controls elevation and pushes the Deity away from objectives.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:3, threatWeight:3 } },
    { id:'MG-CREATURE-024', name:'Valkyr Storm-Caller', combatRole:'GUARDIAN', abilities:['Gale Mark','Oath Mend','Raven Sight'], mapBehavior:'Buffs melee units and marks exposed Deity routes.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    // Olympian (MG-FACTION-003)
    { id:'MG-CREATURE-025', name:'Olympian Laurel Hoplon Guard', combatRole:'GUARDIAN', abilities:['Phalanx Pin','Laurel Guard','Trial Rebuke'], mapBehavior:'Creates defensive walls and protects ranged allies.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:88, damage:16, range:1, armor:7, resistance:5, movement:3, threatWeight:2 } },
    { id:'MG-CREATURE-026', name:'Oracle Glass Archer', combatRole:'CONTROLLER', abilities:['Predicted Shot','Skyline Volley','Fate Thread Mark'], mapBehavior:'Fires from elevation and rewards closing lanes.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:5, resistance:5, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-027', name:'Bronze Chimera Trialbeast', combatRole:'ELITE', abilities:['Tri-Aspect Maul','Arena Flame','Serpent Feint'], mapBehavior:'Combines melee, hazard, and debuff pressure.', aiProfile:{ archetype:'ELITE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:4, threatWeight:3 } },
    // Kami (MG-FACTION-004)
    { id:'MG-CREATURE-028', name:'Kami Torii Foxfire Runner', combatRole:'HUNTER', abilities:['Foxfire Dash','Mirror Feint','Seal Nip'], mapBehavior:'Flanks the Deity and baits wasted AoE.', aiProfile:{ archetype:'HUNTER' }, stats:{ hp:88, damage:16, range:1, armor:5, resistance:5, movement:5, threatWeight:2 } },
    { id:'MG-CREATURE-029', name:'Mirror-Lake Onryo', combatRole:'DISRUPTOR', abilities:['Grudge Pull','Reflection Hex','Drowned Silence'], mapBehavior:'Pulls the Deity toward hazards and slows recovery.', aiProfile:{ archetype:'DISRUPTOR' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-030', name:'Storm Komainu Guardian', combatRole:'GUARDIAN', abilities:['Threshold Guard','Thunder Bite','Ward Howl'], mapBehavior:'Guards objective gates and protects support spirits.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:3, threatWeight:3 } },
    // Tuatha (MG-FACTION-005)
    ...
    { id:'MG-CREATURE-032', name:'Bog Sidhe Hexer', combatRole:'GUARDIAN', abilities:['Name-Tax Hex','Mire Veil','Debt Ward'], mapBehavior:'Debuffs priority abilities and shields allies.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-033', name:'Ancient Root Ogre', combatRole:'BRUTE', abilities:['Root Slam','Living Wall','Spore Stagger'], mapBehavior:'Blocks chokepoints and creates destructible terrain pressure.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:3, threatWeight:3 } },
    // Empyrean (MG-FACTION-006)
    { id:'MG-CREATURE-034', name:'Empyrean Radiant Censor', combatRole:'CONTROLLER', abilities:['Censure Beam','Aura Sight','Contradiction Brand'], mapBehavior:'Marks the Deity and supports disciplined ranged fire.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:5, resistance:5, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-035', name:'Throne-Spear Adjudicator', combatRole:'BRUTE', abilities:['Adjudicator Thrust','Radiant Interdict','Edict Step'], mapBehavior:'Uses reach and zone denial to pin Deity movement.', aiProfile:{ archetype:'BRUTE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:4, threatWeight:3 } },
    { id:'MG-CREATURE-036', name:'Seraphic Bastion Servitor', combatRole:'GUARDIAN', abilities:['Bastion Wing','Sanction Shield','Order Pulse'], mapBehavior:'Protects ranged units and counters reckless cleaves.', aiProfile:{ archetype:'GUARDIAN' }, stats:{ hp:88, damage:16, range:1, armor:7, resistance:5, movement:3, threatWeight:2 } },
    // Infernal Dominion (MG-FACTION-007)
    { id:'MG-CREATURE-037', name:'Infernal Cinder-Chain Bailiff', combatRole:'CONTROLLER', abilities:['Chain Writ','Debt Drag','Cinder Lock'], mapBehavior:'Drags the Deity into bad positions and taxes movement.', aiProfile:{ archetype:'CONTROLLER' }, stats:{ hp:88, damage:16, range:3, armor:7, resistance:8, movement:4, threatWeight:2 } },
    { id:'MG-CREATURE-038', name:'Brass Hellhound Packfiend', combatRole:'HUNTER', abilities:['Pack Lunge','Scent of Debt','Ash Snap'], mapBehavior:'Fast pressure unit that surrounds and exposes the Deity.', aiProfile:{ archetype:'HUNTER' }, stats:{ hp:88, damage:16, range:1, armor:5, resistance:5, movement:5, threatWeight:2 } },
    { id:'MG-CREATURE-039', name:'Black Iron Executor', combatRole:'ELITE', abilities:['Execution Arc','Sentence Mark','Furnace Guard'], mapBehavior:'Elite burst threat with telegraphed lethal windows.', aiProfile:{ archetype:'ELITE' }, stats:{ hp:126, damage:22, range:1, armor:7, resistance:5, movement:4, threatWeight:3 } }
  ];
  ...
  const STARTER_DEITY_IDS = ['MG-DEITY-001','MG-DEITY-006','MG-DEITY-009'];
  const AWAKENING_BEATS = [
    ...
    { step:9, id:'SECOND_DEITY_ENCOUNTER', lesson:'Second Deity encounter', missionBeat:'A second Deity appears through a story battlefield event and solves a problem differently.', playableSystem:'specialist contrast', milestone:'AWAKENING_BEAT_09' },
    { step:10, id:'DEITY_TRIAL_HANDOFF', lesson:'Divine Trial', missionBeat:'Temporarily pilot a second Deity with its mechanic, sample gear, and Divine Ascension.', playableSystem:'temporary trial', milestone:'AWAKENING_BEAT_10' },
  ...
  function starterDeitys(){ const rows=STARTER_DEITY_IDS.map(deityById).filter(Boolean); return rows.length?rows:(deities()[0]?[deities()[0]]:[]); }
  function ensureOnboarding(player){
    player.onboarding ||= { status:'AWAKENING', starterDeityId:null, milestones:[], awakeningMissionId:null, currentBeat:1, completedBeatIds:[], starterChoiceReason:null, fullRosterHidden:true };
    player.onboarding.milestones ||= [];
    player.onboarding.completedBeatIds ||= [];
    player.onboarding.status ||= player.onboarding.starterDeityId?'COMPLETE':'AWAKENING';
  ...
  const TRIAL_DEITY_IDS = ['MG-DEITY-005','MG-DEITY-005','MG-DEITY-010'];
  const TRIAL_MODES = [
  ...
  function trialDeitys(){ const rows=TRIAL_DEITY_IDS.map(deityById).filter(Boolean); return rows.length?rows:deities().slice(1,4); }
  function ensureTrials(player){
    player.deityTrials ||= { attempts:[], completions:[], bestScores:{}, trialFavor:0, showcasedDeityIds:[], conversions:[] };
    player.deityTrials.attempts ||= [];
    player.deityTrials.completions ||= [];
    player.deityTrials.bestScores ||= {};
    player.deityTrials.showcasedDeityIds ||= [];
  ...
  const AssetManager = {
    ...
    visibleArtReport(player=ensurePlayerState()){
      ...
      const selected=player.selectedDeities.map(id=>this.forEntity(id,'DEITY_PRESENTATION'));
    ...
    preloadVisible(){ const p=ensurePlayerState(); const preload=[...p.selectedDeities.map(id=>this.forEntity(id,'DEITY_PRESENTATION')), this.forEntity(p.campaignProgress.currentFactionId,'COMMAND_HUB_BACKGROUND'), this.forEntity(p.campaignProgress.currentFactionId,'GATE')]; state.perf.assetPreloadCount+=preload.length; return preload; }
  };
  ...
  function canonicalDefaultPlayer(){
    ...
    base.playerId ||= 'MG-PLAYER-LOCAL-001'; base.playerName ||= 'Gate Commander'; base.level ||= 1;
    ...
    base.quests ||= [{ id:'MG-QUEST-FIRST-GATE', title:'Open the First Gate', objective:'Launch the first campaign mission.', status:'ACTIVE', progress:0, target:1 }];
    base.onboarding ||= { status:'AWAKENING', starterDeityId:null, milestones:['GATE_SIGNAL_DETECTED'], awakeningMissionId:firstMission, currentBeat:1, completedBeatIds:[], starterChoiceReason:null, fullRosterHidden:true };
    base.notifications ||= [];
  ...
  function normalizeProgression(player){
    ...
    player.raidProgress.masteryByDeityId ||= {};
    player.raidProgress.weeklyTokens ||= {};
    player.raidProgress.materialHistory ||= [];
    ensureResource(player,'MG-RES-GATE-SHARDS','Gate Shards',0);
    ensureResource(player,'MG-RES-ASCENSION-EMBER','Ascension Ember',0);
    ensureResource(player,'MG-RES-DEITY-RELICS','Deity Relics',0);
    ensureResource(player,'MG-RES-RAID-TOKENS','Raid Tokens',0);
    ensureResource(player,'MG-RES-SIGNATURE-ALLOY','Signature Alloy',0);
    ensureResource(player,'MG-RES-MASTERY-SEALS','Mastery Seals',0);
    ensureResource(player,'MG-RES-TRIAL-FAVOR','Trial Favor',0);
  ...
  function deriveNotifications(player){
    ...
    if(current && !(player.campaignProgress.completedMissionIds||[]).includes(current.id)) notes.push({ id:'MG-NOTE-CONTINUE-CAMPAIGN', type:'campaign', label:'Campaign objective waiting', route:'battle' });
    if((player.campaignProgress.pendingRewards||[]).length) notes.push({ id:'MG-NOTE-REWARD', type:'reward', label:`${player.campaignProgress.pendingRewards.length} reward cache${player.campaignProgress.pendingRewards.length===1?'':'s'} pending`, route:'command' });
    for(const id of player.selectedDeities||[]){ const t=deityById(id); if(t && (t.abilityDetails||[]).some(a=>a.unlockLevel<=player.level+1 && a.unlockLevel>player.level)) notes.push({ id:`MG-NOTE-${id}-UPGRADE`, type:'deity', label:'Deity upgrade soon', route:'deitys', entityId:id }); }
    return notes;
  }
  ...
  function getNextRecommendedAction(player=ensurePlayerState()){
    ...
    if(!player.selectedDeities?.length) return { label:'SELECT DEITY', route:'deitys', reason:'A valid Deity must be selected.' };
  ...
  function createRewardCache(player, mission, battle){
    ...
    return { id:`MG-REWARD-${mission.id}-${firstClear?'FIRST':'REPLAY'}-${Date.now()}`, missionId:mission.id, title:mission.title||mission.id, firstClear, missionType, status:'PENDING', summary:firstClear?'First-clear cache secured.':'Replay cache secured under anti-exploit limits.', grants:{ gateShards:shardAmount, ascensionEmber:emberAmount, deityRelics:relicAmount, xp }, battleSummary:summarizeBattle(battle), createdAt:new Date().toISOString() };
  }
  ...
  function applyRewardCache(player, cache){
    normalizeProgression(player);
    ensureResource(player,'MG-RES-GATE-SHARDS','Gate Shards').amount += cache.grants.gateShards||0;
    ensureResource(player,'MG-RES-ASCENSION-EMBER','Ascension Ember').amount += cache.grants.ascensionEmber||0;
    ensureResource(player,'MG-RES-DEITY-RELICS','Deity Relics').amount += cache.grants.deityRelics||0;
    ensureResource(player,'MG-RES-RAID-TOKENS','Raid Tokens').amount += cache.grants.raidTokens||0;
    ensureResource(player,'MG-RES-SIGNATURE-ALLOY','Signature Alloy').amount += cache.grants.signatureAlloy||0;
    ensureResource(player,'MG-RES-MASTERY-SEALS','Mastery Seals').amount += cache.grants.masterySeals||0;
    ensureResource(player,'MG-RES-TRIAL-FAVOR','Trial Favor').amount += cache.grants.trialFavor||0;
  ...
  function createRaidAttempt(player,tier='RAID_NORMAL'){
    ...
    const attempt={ id:`MG-RAID-ATTEMPT-${Date.now()}`, raidId:'MG-RAID-001', bossId:boss.id, name:boss.name, tier, activeDeityId:player.selectedDeities?.[0], stageIndex:0, status:'ACTIVE', score:0, damageTaken:0, turns:0, resolvedStages:[], modifiers:['Ruptured Gate Lanes','Reaction Discipline'], startedAt:new Date().toISOString(), stages };
  ...
  function applyRaidMastery(player, attempt, economy){
    normalizeProgression(player); const deityId=attempt.activeDeityId||player.selectedDeities?.[0]||'UNKNOWN_DEITY';
    const row=player.raidProgress.masteryByDeityId[deityId] ||= { deityId, clears:0, bestScore:0, masteryXp:0, seals:0 };
    ...
    player.raidProgress.materialHistory.unshift({ raidId:attempt.raidId, tier:attempt.tier, deityId, quality:economy.quality, firstClear:economy.firstClear, grants:economy.grants, at:new Date().toISOString() });
  ...
  function completeRaidAttempt(player, attempt){
    ...
    const cache={ id:`MG-REWARD-${attempt.raidId}-${attempt.tier}-${Date.now()}`, missionId:attempt.raidId, title:`${attempt.name} Raid Cache`, firstClear:economy.firstClear, missionType:attempt.tier, status:'PENDING', summary:`${economy.quality}-grade raid economy cache: mastery credit, capped replay payout, and non-pay-to-win materials.`, grants:economy.grants, raidSummary:{...completion, mastery}, economy, createdAt:new Date().toISOString() };
  ...
  function createTrialAttempt(player, deityId=trialDeitys()[0]?.id){
    normalizeProgression(player); const deity=deityById(deityId)||trialDeitys()[0]||deityById(player.selectedDeities?.[0]);
    const mode=TRIAL_MODES[(player.deityTrials.attempts.length||0)%TRIAL_MODES.length];
    const attempt={ id:`MG-TRIAL-ATTEMPT-${Date.now()}`, trialId:'MG-TRIAL-001', deityId:deity?.id, deityName:deity?.name||'Trial Deity', factionId:deity?.factionId||'MG-FACTION-001', mode:mode.id, modeLabel:mode.label, tempLoadout:{ relic:'Borrowed Solar Reliquary', gearScore:72, expires:'END_OF_TRIAL' }, status:'ACTIVE', score:0, turns:0, damageTaken:0, lessons:['Read battlefield fit','Test unique mechanic','Compare collection reason'], startedAt:new Date().toISOString() };
    state.trial=attempt; player.deityTrials.attempts.unshift({ id:attempt.id, deityId:attempt.deityId, mode:attempt.mode, status:'ACTIVE', startedAt:attempt.startedAt }); player.deityTrials.attempts=player.deityTrials.attempts.slice(0,8); savePlayerState(); log('trial','Trial attempt started',{deityId:attempt.deityId,mode:attempt.mode}); return attempt;
  }
  function resolveTrialAttempt(player, attempt, approach='BALANCED'){
    ...
    const deity=deityById(attempt.deityId)||{}; const aggressive=approach==='AGGRESSIVE', studied=approach==='STUDIED';
    ...
    const firstShowcase=!player.deityTrials.showcasedDeityIds.includes(attempt.deityId); if(firstShowcase) player.deityTrials.showcasedDeityIds.push(attempt.deityId);
    ...
    player.deityTrials.trialFavor += favor; ensureResource(player,'MG-RES-TRIAL-FAVOR','Trial Favor').amount += favor;
    player.deityTrials.bestScores[attempt.deityId]=Math.max(player.deityTrials.bestScores[attempt.deityId]||0, score);
    const completion={ id:attempt.id, trialId:attempt.trialId, deityId:attempt.deityId, deityName:attempt.deityName, mode:attempt.mode, quality, score, firstShowcase, favor, completedAt:attempt.completedAt };
    ...
    player.factionTrials.bestScores[attempt.factionId]=Math.max(player.factionTrials.bestScores[attempt.factionId]||0, score); player.factionTrials.completions.unshift({ factionId:attempt.factionId, deityId:attempt.deityId, score, quality, at:attempt.completedAt }); player.factionTrials.completions=player.factionTrials.completions.slice(0,12);
    const cache={ id:`MG-REWARD-${attempt.trialId}-${attempt.deityId}-${Date.now()}`, missionId:attempt.trialId, title:`${attempt.deityName} Trial Cache`, firstClear:firstShowcase, missionType:'DEITY_TRIAL', status:'PENDING', summary:`${quality}-grade temporary loadout showcase. Trial Favor persists; borrowed gear does not.`, grants:{ gateShards:shardGrant, ascensionEmber:0, deityRelics:firstShowcase?1:0, raidTokens:0, signatureAlloy:0, masterySeals:0, trialFavor:favor, xp }, trialSummary:completion, createdAt:new Date().toISOString() };
    player.campaignProgress.pendingRewards.push(cache); player.notifications=deriveNotifications(player); state.trial=null; savePlayerState(); log('trial','Trial completed',{deityId:attempt.deityId,quality,score,rewardId:cache.id}); return cache;
  }
  ...
  function bottomNav(){
    const tabs = [{id:'battle',label:'BATTLE'},{id:'battlefields',label:'FIELDS'},{id:'deitys',label:'DEITIES'},{id:'trials',label:'TRIALS'},{id:'command',label:'COMMAND'}];
    const routeMap={battle:'hub',battlefields:'battlefields',deities:'deitys',trials:'trials',raid:'raid',command:'command'};
  ...
  function deitySilhouette(t, i){
    const asset=AssetManager.forEntity(t.id,'DEITY_PRESENTATION'); const rarity=t.rarity||'Unknown'; const power=t.stats?.combatPower||t.stats?.attack||'—';
  ...
  function trialsScreen(){
    const player=ensurePlayerState(); normalizeProgression(player); const trials=ensureTrials(player); const active=state.trial?.status==='ACTIVE'; const options=trialDeitys().map(t=>`<article class="rounded-3xl border border-neutral-800 bg-neutral-900 p-4"><p class="text-xs font-black uppercase tracking-[.22em] text-primary">${esc(t.faction)} · ${esc(t.rarity)} Trial</p><h3 class="mt-1 text-2xl font-black">${esc(t.name)}</h3><p class="mt-2 text-sm text-neutral-300">${esc(t.preferredBattlefield||t.collectionReason||t.role)}</p><p class="mt-2 rounded-2xl bg-neutral-950 p-3 text-xs text-neutral-500">${esc(t.notMandatoryRule||'Recommended advantage, never an ownership lock.')}</p><button onclick="TGHub.startTrial('${esc(t.id)}')" class="mt-4 w-full rounded-2xl bg-primary px-4 py-3 font-black text-white">START TEMP TRIAL</button></article>`).join('');
  ...
  function buildBattle(){
    ...
    const ids=(mission.enemyIds||mission.enemies||['MG-CREATURE-001','MG-CREATURE-001','MG-CREATURE-002']).slice(0,3);
    const enemySet=ids.map(id=>creatureById(id)).filter(Boolean); if(!enemySet.length) enemySet.push({id:'MG-CREATURE-FALLBACK',name:'Hollow Wretch',combatRole:'PRESSURE',stats:{hp:18,damage:6,armor:4,resistance:3,range:1,movement:1}});
    ...
    state.battle=createBattleState({ battleId:`MG-HUB-${mission.id||'FIRST-GATE'}`, missionId:mission.id||'MG-F01-C01-M01', deity, enemies:scaledEnemySet, objectives, scaling }); log('battle','Playable battle initialized',{ missionId:mission.id, deity:deity?.id, scaling });
  }
  ...
  const api={
    ...
    openTab(tab){ state.selectedTab=tab; const map={battle:'hub',battlefields:'battlefields',deities:'deitys',trials:'trials',raid:'raid',gates:'gates',codex:'codex',command:'command'}; setRoute(map[tab]||'hub'); },
    chooseStarter(id){
      const p=ensurePlayerState(); const starters=new Set(starterDeitys().map(t=>t.id));
      if(!starters.has(id)){ log('onboarding-error','Rejected non-starter Deity',{id}); render(); return; }
      p.selectedDeities=[id]; const ob=ensureOnboarding(p); ob.starterDeityId=id; ob.starterChoiceReason=starterDeitys().find(t=>t.id===id)?.role || 'Canon starter'; completeAwakeningBeat(p,1); if(!ob.milestones.includes('STARTER_DEITY_BOUND')) ob.milestones.push('STARTER_DEITY_BOUND'); p.notifications=deriveNotifications(p); savePlayerState(); AudioManager.play('deity_selected'); render();
    },
    ...
    focusDeity(id){ state.focusDeityId=id; setRoute('deitys'); },
    ...
    startTrial(deityId){ const p=ensurePlayerState(); createTrialAttempt(p,deityId); state.selectedTab='trials'; setRoute('trials'); },
