...
assert.equal(summary.deity.id, 'MG-DEITY-001');
assert.equal(summary.enemiesRemaining, 3);

const player = { id: 'MG-QA-PLAYER' };
...
let platformState = platform.createPlatformProfile({ playerId: 'MG-QA-PLAYER', starterDeityId: 'MG-DEITY-003' });
platformState = platform.completeMission(platformState, 'MG-F01-C01-M01', { accountXp: 20, currencies: { sunshards: 10 } });
...
assert.ok(lookup.deityById.has('MG-DEITY-001'));
assert.ok(lookup.creatureById.size >= 1);

const deitySchema = JSON.parse(fs.readFileSync('schemas/deity.schema.json', 'utf8'));
assert.equal(validateContract(dataset.deities[0], deitySchema, 'firstDeity').ok, true);
