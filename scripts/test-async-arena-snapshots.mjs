...
const mini = fs.readFileSync('mini-app/galaxy-reapers-ascension.html', 'utf8');
...
for (const token of ['Asynchronous Titan Challenge', 'Snapshot Model Ready', 'one-active defender Titan', 'No live PvP']) assert.ok(mini.includes(token), `mini missing ${token}`);
