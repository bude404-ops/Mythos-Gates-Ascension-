...
const auditByDeity = new Map((audit.entries || []).map(entry => [entry.deityId, entry]));
...
if (!Array.isArray(audit.entries) || audit.entries.length !== deitys.length) fail(`Audit entry coverage mismatch: ${audit.entries?.length || 0}/${deitys.length}`);
if (prompts.length !== deitys.length) fail(`Deity prompt coverage mismatch: ${prompts.length}/${deitys.length}`);
...
for (const deity of deitys) {
  ...
  if (!deity.deityArtDna) fail(`${deity.id}: missing deityArtDna`);
  for (const field of ['mythology','realm','civilization','cultureRule','beautyStandard','realmAnatomy','identityFace','identityBody','roleSilhouette','signatureWeapon','auditStatus']) {
    if (!deity.deityArtDna?.[field]) fail(`${deity.id}: deityArtDna missing ${field}`);
  }
  ...
  if (!prompt) fail(`${deity.id}: missing deity art prompt`);
    ...
    if (!prompt.prompt.includes(deity.deityArtDna.signatureWeapon)) fail(`${deity.id}: prompt missing signature weapon`);
  ...
  const entry = auditByDeity.get(deity.id);
  ...
  const face = deity.deityArtDna?.identityFace;
  const body = deity.deityArtDna?.identityBody;
  const weapon = deity.deityArtDna?.signatureWeapon;
...
if ((audit.summary?.pass || 0) + (audit.summary?.refine || 0) + (audit.summary?.redesign || 0) !== deitys.length) fail('Audit summary status counts do not cover all deities');
if (!audit.summary?.highestPriorityDeityId || !auditByDeity.has(audit.summary.highestPriorityDeityId)) fail('Highest-priority Deity recommendation missing or invalid');
if (!exists('docs/lore/DEITY_ART_IDENTITY_AUDIT.md')) fail('Missing Markdown art identity audit document');
