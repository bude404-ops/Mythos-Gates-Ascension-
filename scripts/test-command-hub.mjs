...
const metrics = await page.evaluate(() => ({ route: window.TGHub?.state?.route, selected: window.TGHub?.state?.player?.selectedTitans?.length, selectedTitanId: window.TGHub?.state?.player?.selectedTitans?.[0], onboarding: window.TGHub?.state?.player?.onboarding, notifications: window.TGHub?.state?.player?.notifications?.length, pendingRewards: window.TGHub?.state?.player?.campaignProgress?.pendingRewards?.length || 0, raidClears: window.TGHub?.state?.player?.raidProgress?.completions?.length || 0, raidBest: window.TGHub?.state?.player?.raidProgress?.bestScores?.['TG-RAID-001:RAID_NORMAL'] || 0, raidMastery: Object.values(window.TGHub?.state?.player?.raidProgress?.masteryByTitanId||{})[0] || null, raidCache: (window.TGHub?.state?.player?.campaignProgress?.pendingRewards||[]).find(r=>r.missionId==='TG-RAID-001') || null, economyVisible: document.body.innerText.includes('Raid') && document.body.innerText.includes('Alloy') && document.body.innerText.includes('Mastery') }));
await browser.close();
server.close();
