...
const metrics = await page.evaluate(() => ({ route: window.TGHub?.state?.route, selected: window.TGHub?.state?.player?.selectedTitans?.length, notifications: window.TGHub?.state?.player?.notifications?.length, pendingRewards: window.TGHub?.state?.player?.campaignProgress?.pendingRewards?.length || 0, commandText: document.body.innerText.includes('Mission clears now create save-backed reward caches') }));
await browser.close();
server.close();
assert.equal(errors.length, 0, errors.join('\n'));
assert.equal(metrics.route, 'command');
assert.equal(metrics.pendingRewards, 1);
assert.ok(metrics.commandText);
assert.ok(metrics.selected >= 1);
console.log(JSON.stringify({ ok: true, commandHubSmoke: 'PASS', route: metrics.route, selectedTitans: metrics.selected, notifications: metrics.notifications, pendingRewards: metrics.pendingRewards, viewport: '390x760' }, null, 2));
