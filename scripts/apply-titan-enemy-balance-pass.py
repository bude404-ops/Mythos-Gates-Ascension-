...
ROOT = '/home/reaper/repos/Titans-Gate-sync'
...
titans = read_json(os.path.join(DATA, 'titans.json'))
creatures = read_json(os.path.join(DATA, 'creatures.json'))
roster = read_json(os.path.join(DATA, 'solo-titan-roster-redesign.json'))
roster_by_id = {t['id']: t for t in roster.get('titans', [])}
role_matrix = read_json(os.path.join(DATA, 'titan-role-matrix.json'))
role_by_id = {t['id']: t for t in role_matrix.get('titans', [])}
...
balance_rows=[]
for i,t in enumerate(titans, start=1):
...
write_json(os.path.join(DATA,'titans.json'), titans)
for t in titans:
    write_json(os.path.join(ROOT,'titans',f"{t['id']}.json"), t)
...
write_json(os.path.join(DATA,'titan-enemy-balance-pass.json'), balance_doc)
...
print(json.dumps({'updatedTitans':len(titans),'updatedEnemies':len(creatures),'balanceDoc':'data/titan-enemy-balance-pass.json','titanPowerMin':balance_doc['titanBudget']['combatPowerMin'],'titanPowerMax':balance_doc['titanBudget']['combatPowerMax']}, indent=2))
